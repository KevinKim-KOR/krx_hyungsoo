"""POC2 Step 5D Cleanup — holdings draft / market enrichment / message_text 회귀 테스트.

분리 출처: tests/test_poc1_loop.py (Step 5D 이전 단일 파일).
포함 범위:
- holdings 입력/저장/검증 (POC2 Step1 ~ Step1A)
- market_cache / Naver fetch / enrich (POC2 Step2)
- draft_message Step2B 요약형 / 길이 방어
- account_group (POC2 Step2C)
- message_text 단일 소스 / Run.message_text (POC2 Step2D)
