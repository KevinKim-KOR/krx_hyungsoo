...
def run_backtest(
    ...
    result = runner.run(
        ...
        contextual_guard_params=contextual_guard_params,
        ...
        ml_penalty_weight=ml_penalty_weight,
...
def format_result(
    ...
    meta = {
        ...
        **_build_allocation_meta(result),
        ...
        # P209-STEP9C Track A Contextual Guard meta.
        "tracka_guard_experiment_name": result["tracka_guard_experiment_name"],
        "tracka_baseline_label": result["tracka_baseline_label"],
        "tracka_guard_mode": result["tracka_guard_mode"],
        "tracka_pre_entry_guard_hits_total": result[
            "tracka_pre_entry_guard_hits_total"
        ],
        "tracka_early_stop_hits_total": result["tracka_early_stop_hits_total"],
        "tracka_guard_exhausted_count": result["tracka_guard_exhausted_count"],
        "tracka_promoted_total": result["tracka_promoted_total"],
        "tracka_avg_candidates_before_guard": result[
            "tracka_avg_candidates_before_guard"
        ],
        "tracka_avg_candidates_after_guard": result[
            "tracka_avg_candidates_after_guard"
        ],
        "tracka_pre_entry_guarded_by_rebalance_date": result[
            "tracka_pre_entry_guarded_by_rebalance_date"
        ],
        "tracka_early_stop_events_by_date": result["tracka_early_stop_events_by_date"],
...
def run_cli_backtest(
    ...
    if params.get("universe_mode") == "dynamic_etf_market" and enable_regime:
        ...
        # OPTIONAL: holding_structure_experiments 는 dynamic_etf_market 에서
        # 반드시 존재하지만 None 일 수는 없음 (param_loader 검증). 명시적 분기.
        _hs_exps_raw = params.get("holding_structure_experiments")
        _hs_exps = _hs_exps_raw if _hs_exps_raw is not None else []
    ...
    # P209-STEP9C FIX: contextual_guard sweep 을 evidence 생성 이전에 실행.
    _tracka_experiments = params.get("tracka_contextual_guard_experiments")
    ...
    if (
        ...
        try:
            from app.backtest.reporting.contextual_guard_compare import (
                run_contextual_guard_sweep,
            )
    ...
    # OPTIONAL: universe_mode 가 dynamic_etf_market 인 경우에만 evidence 작성.
    if params.get("universe_mode") == "dynamic_etf_market":
        try:
