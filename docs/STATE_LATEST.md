...
최종 업데이트: 2026-08-19 (**보유와 비교 — 좌우 2열 배치 정정 + 선택 보유 상세 채움** · 사용자 실화면 확인 완료 · **검증자 미착수**. 직전 08-17 A11 라운드는 `VERIFIED`)
...
**다음**: 중복률 탭(`OverlapTab`) 카드 전환. 사용자 확정 — ① ETF 이름 표시(draft 에서, 백엔드
무변경) ② `최고 비중` 칸 포함 ③ 정렬 기본값 중복률 높은순. 실화면 확인 전 **구성종목 수집
1회 필요**(캐시가 `2026-07-03` 에서 멈춰 있어 지금 열면 빈 화면 · 외부 조회라 지시 대기).
