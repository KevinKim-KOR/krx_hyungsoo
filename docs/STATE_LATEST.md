...
**`DEF-INTRADAY-POLICY-ACTIVATION-PATH`** — `DESIGN_REQUIRED` (2026-09-21).
...
~~`DEF-INTRADAY-SUMMARY-WHEN-DISABLED`~~ **해소** (02C-OPS-03, 2026-09-22) —
비활성이면 줄을 붙이지 않는다. 아래는 당시 기록이다.

**`DEF-INTRADAY-SUMMARY-WHEN-DISABLED`** — `DESIGN_REQUIRED` (2026-09-21).
...
**⚠ 전체 pytest 실패 1건 → 1차·2차 수정 모두 REJECTED → A11 로 운영 원인 제거**

**1차(2026-08-17 오전)**: 원인을 *교차 테스트 오염*으로 보고 픽스처 teardown 에 스레드 `join` 을 넣었다. 전체 pytest 1회 통과를 근거로 "해소" 라고 보고했다. **이 보고는 성립하지 않았다** — 검증자가 직접 대상 32건 1차 실행에서 **동일 실패를 재현**했고(31 passed/1 failed, 재실행 시 32 passed), 종합 판정 **REJECTED**. 1회 통과는 경쟁 상태 소멸의 증거가 아니다.

**2차(재작업) — 진짜 원인은 테스트 내부 경쟁이었다.** `_run_job` 은 상태 파일에 **두 번** 쓴다: ① `status="running"` → `_write_status` ② `_mark_step(feature)` → `_write_status`. 테스트가 파일을 폴링해 **①만 보고** 통과 판정한 뒤 두 번째 `start` 를 호출했고, 그때 **②가 파일을 비우고 쓰는 중**이면 빈 파일을 읽어 `JobStatusCorruptedError` 가 났다(운영 `_write_status` 가 `open("w")` 직후 기록이라 비원자적 — **별도 과제**).

**조치**: 중간 상태 폴링 → **단계 진입 이벤트(`entered_feature`) 대기**로 교체. ②가 끝난 뒤 set 되므로 그 시점엔 쓰기가 없다 — 결정적으로 경쟁이 사라진다. teardown 은 `join` 후 **`is_alive()` 를 확인**하고, 살아 있으면 조용히 락을 풀지 않고 **테스트를 실패시킨다**(검증자 B-6).
...
| 직접 대상 3파일 | **12회** | 통과 12 · 실패 0 |
| 백엔드 전체 | **3회** | **1142 passed · 실패 0건** (3회 모두) |

**A11 — 운영 경쟁 조건 제거 (2026-08-17)**: 검증자 2차 REJECTED 사유는 **"테스트가 경쟁 구간을 피했을 뿐 운영 원인 미해소"** 였고 이는 정확했다. 사용자 A11 승인으로 `app/ml_job_runner.py::_write_status()` 만 최소 수정 — 같은 디렉터리 임시 파일에 완성본을 쓰고 **단일 `os.replace`** 로 교체한다(POSIX·Windows 모두 원자적). active 정본 1개 유지 · 읽기 경로·스키마·시그니처·API·DB 무변경 · 교체 실패 시 이전 active 보존 + 임시 파일 정리 · 실제 손상 시 `JobStatusCorruptedError` 계약 유지.

**경쟁 구간을 의도적으로 여는 회귀 테스트 5건 추가**(반복 통과로는 부족하다는 지적 반영): `os.replace` 를 교체 직전에 멈춰 창을 연 뒤 ① 읽기가 **이전 완성본**을 보는지 ② 중복 start 가 `already_running` 인지(손상 오류 아님) ③ 교체 실패 시 이전 active 보존·임시 파일 0건 ④ 실제 손상은 여전히 fail-loud ⑤ 임시 파일이 같은 디렉터리인지.

**테스트가 결함을 실제로 잡는지 역검증**: 옛 비원자적 구현으로 되돌리면 **4건이 실패**하고, 원자적 구현에서는 전부 통과한다. 검증자가 쓴 강제 재현 방식으로도 손상 오류가 재현되지 않는다.

**신규 필드 직접 테스트 3건 추가**(검증자 지적) 중 1건이 **제 누락을 잡았다** — `matched_candidate is None` 경로에 신규 키를 넣지 않아 같은 함수가 분기별로 다른 키 집합을 반환하고 있었다(API 모델 기본값이 덮어써 응답에는 미노출). 키 집합을 맞췄다.

> **개발자 판단 오류 3회 기록**: (1) "제 변경과 무관" (2) "flaky 라 어쩌다 한 번" (3) "픽스처 수정으로 해소" — **셋 다 측정이 부족한 상태의 결론**이었고, 각각 베이스라인 대조·재현·검증자 재현으로 뒤집혔다. 특히 (3) 은 (2) 와 같은 오류(1회 통과를 근거로 삼음)를 반복한 것이다. 원인 규명을 지시한 사용자 판단이 맞았다.
...
**상태**: 구현 완료 · **검증자 대상** — 앞선 UI 라운드와 달리 **`app/` 4개 파일 변경**.
결과서 `docs/ai_result/POC3/POC3-CONSTITUENTS_DEPTH30_AND_CHANGE_RATE_REMOVAL_RESULT.md`.

**확정문**: 같은 source·endpoint·스키마를 유지하며 ETF당 최대 30건 **한 번** 수집.
구성종목 탭은 30건 표시, **중복률은 rank 1~10만 사용해 `Top 10 기준` 계약 유지**.
ETF 1회 상한 10개·시간 예산·캐시 정책 유지. 등락률은 신규 수집 없이 **열·안내문 제거**,
개별주 등락률은 BACKLOG.

**착수 전 중단 조건 확인** — 확정문이 *"30건 확보에 추가 호출이 필요하면 중단·보고"* 를
걸어놨고, 코드는 `pageSize=20` 고정이라 응답을 받아보기 전에는 알 수 없었다.
**사용자 허락을 받고 진단 호출 1회** 실행 →
`HTTP 200 · 30건 · referenceDate 2026-08-19 · componentCount 200`.
**중단 조건 해당 없음** (ETF당 1회 호출로 30건).

**⚠ 보고 정확성 위반** — 질의 문서에 적은 *"캐시에 ETF당 30건 저장된 과거분"* 은
**사실이 아니었고**(스냅샷 50개 전부 10행 · 최대 rank 10 · 날짜 3개를 하나로 착각),
**그 잘못된 사실이 설계 확정문의 근거로 인용**됐다. 결론은 실측으로 독립 확인돼 영향이
없었으나, 질의 문서·앞선 결과서·본 문서 세 곳에 정정을 남겼다.

**구현 핵심**: 외부 호출 상한(ETF 10개·delay·예산·캐시 우선)은 **그대로**, ETF 안의
깊이만 30. `pageSize` 를 요청 깊이에 맞춰 **1회 호출**. **표시 깊이 ≠ 중복률 깊이**
(중복률·반복 집계는 항상 10 고정 · 응답 `overlap_top_k`). **정확히 10건인 캐시만**
구 정책 잔재로 보고 재수집(그 외 건수는 source 고갈이라 캐시 유지 — 안 그러면 30개
미만 ETF 가 매번 재조회돼 캐시 우선이 깨진다). `upsert` 가 rank 단위라 **축소 덮어쓰기
없음**(회귀 테스트로 고정). 프론트는 제목 `상위 구성종목` · 요약 `상위 N개 표시 · 표시
비중 합계` · **등락률 열·안내문 제거**.

**회귀 테스트 10건** 추가. 작성 중 자체 오류 2건을 잡았다 — 양쪽 ETF 에 같은 티커를
넣어 겹침이 항상 잡히던 것, `hash()` 로 만든 티커가 **실행마다 달라지던** 것.

**검증 실측**: `black` 246 files unchanged · `flake8` 0건 · 백엔드 **1157 passed**
(직전 1147 + 신규 10) · `tsc` 통과 · `eslint` 0건 · `vitest` 167 passed.

**⚠ 실화면 확인 전 구성종목 재수집 필요** — 현재 캐시는 10건 스냅샷이라 화면에 30건이
나오지 않는다. 수집 버튼을 누르면 위 규칙으로 재수집된다.

**⚠ r1 검증자 `REJECTED` → r2 재작업 (2026-08-19)**

지적 4건 전부 타당했다. 핵심은 **깊이를 조회 한 곳만 고치고 정작 사용자가 누르는 수집
버튼(POST)과 직후 재조회(GET)에는 리터럴 `10` 을 남겨둔 것** — 백엔드가 *"10건 요청 +
10건 캐시"* 를 완료로 보므로 **실제 사용자 경로에서는 재수집도 30건 표시도 일어나지
않았다.** 안내문도 `상위 10개 구성종목` 그대로였다. `CLAUDE.md §11` 의 *"먼저 grep 으로
모든 layer 를 열거"* 를 착수 시 지키지 않은 결과다.

**조치**: 리터럴을 없애고 **단일 상수 `CONSTITUENTS_TOP_K = 30`** 으로 모아 수집 POST ·
직후 GET · 마운트 GET · API 기본값 · 안내문 5곳에 일괄 적용. 그 외 —
누락 비중을 `?? 0` 으로 합산하던 것을 **값 있는 것만 합산 + 제외 개수 명시**(전부 없으면
`확인 불가`, `0.00%` 금지), `overlap_top_k ?? 10` fallback 제거(타입에서 optional 제거),
결과서 변경 파일 목록에 빠졌던 `docs/PROGRAM_TRUTH.md` 추가.

**프론트 회귀 6건 신규**(`ConstituentsTab.test.tsx`) — 백엔드 10건이 전부 통과하고도
연결 누락을 못 잡은 것이 B-6 지적이었다. **역검증**: REJECT 된 커밋(`004517f8`)으로
되돌리면 **6건 중 4건 실패**.

**r2 검증 실측**: `tsc` 통과 · `eslint` 0건 · `vitest` **173 passed (16 files)**
(직전 167 + 신규 6). r2 는 프론트 전용이라 백엔드는 r1 의 1157 passed 유효.

**⚠ r2 검증자 `REJECTED` — 기능은 통과, 보고 정확성 2건 (2026-08-19)**

r2 에서 **기능 결함은 해소**됐다(A-1·A-3·B 전 항목 통과 · 위험 NONE · 폭주 NONE).
남은 지적은 **A-2** 뿐이고 둘 다 타당했다 — (a) r2 변경 파일 목록에 **문서 2개 누락**
(커밋은 6개인데 프론트 4개만 적음) (b) `STATE_LATEST` numstat 을 `38/4` 로 적었으나
실제는 **`47/3`**.

**공통 원인**: `git diff --numstat` 을 **커밋 직전에** 재고 그 뒤에도 문서를 계속 고쳤다.
그래서 나중에 만진 파일이 목록에서 빠지고 미리 잰 수치가 커밋과 어긋났다.
**"실측" 이라고 쓴 것이 측정 시점 때문에 사실이 아니게 된 것이다.**

**조치 (r3)**: 수치를 **커밋된 결과에서** `git show --numstat <sha>` 로 다시 뽑아
결과서 §4.1(r1 `004517f8` 16개)·§4.2(r2 `dffac9ef` 6개)를 재작성하고 측정 방법을
§4 머리에 명시했다. **결과서 자신이 변경 대상인 라운드는 수치를 적지 않는다** — 적는
행위가 수치를 바꾸는 자기참조다. 함께 지적된 중복률 테스트 판별력 부족(B-6 참고)에
`overlap_top_k: 7` 케이스를 추가했다(한계도 결과서 §12.4 에 명시).

**r3 검증 실측**: 운영 코드 변경 0건 · `tsc` 통과 · `eslint` 0건 ·
`vitest` **174 passed (16 files)** · 백엔드 diff 0건(r1 의 1157 passed 유효).

**재검증 `VERIFIED` (2026-08-20)** — r1(기능)·r2(보고 정확성) 지적 모두 해소.

**실화면 수집 실측**: `asof 2026-08-20` · ETF 10개 · **총 180행 · 최대 rank 30**.
ETF별 확보 건수는 30 / 28 / 19×3 / 16×2 / 11×3 로 **30건이 찬 것은 1개뿐**이다.
결함이 아니라 **해당 ETF 의 구성종목이 그만큼뿐**이라는 뜻이고(source 고갈), 설계 확정의
*"30건 미만이면 실제 확보된 건수만 표시"* 가 그대로 적용된다. API 응답도 동일
(`top_k 30` · `overlap_top_k 10`). **화면에 10건으로 보인 것은 브라우저 캐시** —
데이터·API 문제가 아니며 강력 새로고침(⌘⇧R)으로 해소된다.

**다음**: `확인 근거` 데이터 영역(`EvidenceDetails`) 카드 전환. **이 컴포넌트는 좌측
메뉴에 단독으로 없다** — `RunPanel` 안에 있고, `RunPanel` 은 **`승인·적용`** 화면과
**`개발·실험용`** 화면에서 쓰인다. 착수 전 실제 구조·영향 범위를 먼저 제시한다.
그 뒤 남은 표 — `AI 투자 세션` 목록 · `ETF 구성종목`의 구성종목 표.

---

### 2026-08-19 (2) 중복률 탭 카드 전환 + 구성종목 탭 식별 줄 (사용자 직접 지시)

**상태**: **DONE — 사용자 실화면 확인 완료.** **검증자 미대상** — 사용자 명시
*"UI변경이기에 검증자를 거치지 않았습니다."* 백엔드·API·수집 경로 변경 **0건**.

**착수 전 정정**: 남은 목록에 `OverlapTab(11)` 로 적혀 있었으나 실제로는 **표 2개(5열 + 4열)**.

**중복률 탭** — 표 2개 → 카드 2벌(`.wb-hrow.compact` 재사용). 사용자 확정 3건 반영:
① **ETF 이름 표시** — 응답의 `etf_name` 은 **저장 시점부터 전부 `NULL`**(네이버 구성종목
API 가 ETF 자기 이름을 주지 않음 · DB 실측 100행). 화면이 이미 들고 있는 **draft 후보
스냅샷**에서 가져온다(`ETFExposureView` → `OverlapTab`·`ConstituentsTab`). draft 에 없는
티커는 티커 그대로 — 이름을 지어내지 않는다. ② **최고 비중** 칸 + 해당 ETF 이름.
③ **정렬 세그먼트 3종**, 기본 중복률 높은순. **겹침 없는 쌍은 0% 로 쓰지 않고**
(`겹치는 종목 없음` 배지) 정렬에서 항상 뒤로.

**구성종목 탭** — summary 줄을 다른 화면 규칙에 맞췄다. `[ok] 0223A0 · 이름 · source · (캐시)`
→ **`이름 [ok] 0223A0 [캐시]`**. source 는 펼친 상세 맨 위로 옮기고 `asof` 를 함께 표시
(이전엔 개별 ETF 기준일을 볼 방법이 상단 요약뿐이었다). 배지는 공용 `.wb-hb` 로 통일
— 이 화면만 테두리 있는 알약이었다. **정상 배지는 사용자 판단으로 유지**(다른 화면은 숨김).

**⚠ 설계 판단 요청 2건 분리** — 작업 중 사용자가 발견했고 개발자가 결정할 수 없다.
`docs/handoff/POC3/POC3_CONSTITUENTS_TOPK_AND_CHANGE_RATE_QUESTION.md`
① **구성종목 상위 10건 제한** — `MAX_TOP_K = 10`, *"외부 fetch 의존성을 강하게 가두기 위한
K6 방어 정책(지시문 §4)"*. **조회가 아니라 저장 시점** 제한. **⚠ 이때 함께 보고한 "캐시에 ETF당 30건 저장된 과거분" 은
오보고였다** — DB 실측 결과 스냅샷 50개가 전부 10행이고 최대 rank 는 10이다. 날짜가 다른
3개 스냅샷을 한 개로 착각했다. 잘못된 사실이 설계 판단 근거에 들어갔다.
③ **등락률 미연결** — 값을 못 가져온 게 아니라 연결한 적 없는 자리(코드 주석·화면 안내문
모두 명시). 채우려면 개별주 시세 조회 신설 → `KILL_SWITCHES §5` 방향.

**검증 실측**: `tsc` 통과 · `eslint` 0건 · `vitest` **167 passed (15 files)**.
백엔드 무변경이라 pytest 재실행 대상 아님.

**다음**: 남은 표 카드 전환 — `AISessionsListTab`(8) · `EvidenceDetails`(8) ·
`ConstituentsTab` 구성종목 표(6). `확인 필요` 탭은 항목 목록이라 전환 여부 미결.

---

### 2026-08-19 추가 라운드 — 배치 정정 + 선택 보유 상세 (사용자 직접 지시)

**상태**: 사용자 실화면 확인 완료 · **검증자 미착수** (직전 A11 `VERIFIED` 범위 밖).

**① 배치 정정** — 사용자가 실화면과 목업이 다른 것을 발견했다. `git show` 로 전환 직전
코드를 실측한 결과 **원래 화면은 2열이 아니라 `1fr 360px` 에 보유/후보가 세로로 쌓인**
형태였고, **목업만 좌우 2열로 그려져 있었다.** 결과서·PROGRAM_TRUTH 에 적었던
*"2열 배치 무변경"* 은 사실이 아니었다(둘 다 정정). 사용자 결정 **(가) 좌우 나란히**
— *"결국은 pc에서 조회할 것이기에"*. `.cmp-twocol`(`auto-fit, minmax(430px,1fr)`) 신설,
선택 상세는 2열 아래 전체 폭으로 이동. `.wb-hrow.compact` CSS 는 이미 2열 전제로
쓰여 있었다(주석: *"좌우 2열 안에 들어가므로"*) — **CSS 는 2열, JSX 만 세로**였던 상태.

**② 선택 보유 상세 채움** — 08-16 에 중복 3줄을 빼고 **대신 채울 것을 넣지 않아** 종목명
한 줄만 남아 있었다. 배치 정정으로 전체 폭에 내려오자 빈 영역이 드러나 사용자가 지적.
`SelectedHoldingDetail.tsx`(신규 217줄)로 **응답에 이미 있고 카드에 없는 값** 7줄을 채웠다
— 보유 내역 · 후보 관계 · 단기 흐름 · KODEX 대비 · NAV 괴리 · 구성종목 겹침 · 확인 근거.
`evidence_notes` 는 백엔드 문장을 그대로 나열한다(새 문구 생성 X). **신규 계산·endpoint·
외부 조회 0건.** 다계좌 평균단가 `계좌별 상이` · `미포함`/`확인 불가`/`확인 전` 3구분 유지.
`DecisionDraftPreviewCard` 는 명시 클릭 계약 그대로.

**검증 실측**: `tsc --noEmit` 통과 · `eslint` 0건 · `vitest` **167 passed (15 files)**.
백엔드 무변경이라 pytest 재실행 대상 아님.

**다음**: 중복률 탭(`OverlapTab`) 카드 전환 → **위 (2) 라운드에서 완료**.

**다음(직전 기록)**: A11 보류 조건 **해제됨**(재검증 `VERIFIED`). 남은 표 카드 전환 대상 — `OverlapTab`(11) · `AISessionsListTab`(8) · `EvidenceDetails`(8) · `ConstituentsTab`(6). 사용자 지시로 **`OverlapTab` 부터** 착수(현재 구조 제시 → 목업 승인 → 구현 → 실화면 확인 순). `확인 필요` 탭은 표가 아니라 항목 목록이라 전환 여부 미결.
...
**상태**: 코드·자동 검증 **DONE** · **사용자 실화면 확인 대기**.

**배경**: 사용자 지시 "ETF 비교하기부터 지금까지 개발했던 형태와 다른 점을 바꿀 준비 및 실행". 표로 남아 있던 화면 중 **가장 넓은 17열**부터 착수.

**탭 구성**: `기본 / 보유와 비교` → **`카드`(기본) / `표` / `보유와 비교`**. 기존 `기본` 라벨을 `카드` 로 변경(같은 데이터의 두 표시 형태가 생겨 "기본" 이 모호해짐). `CompareViewMode` 타입도 `"default"` → `"card" | "table"`.

**17열 분류(버리지 않음)**: 카드 상단 순위·ETF명·**참고점수** / 카드 하단 **티커·시장가·NAV·괴리율**(사용자 지정) / 우측 지표 일간·1개월·3개월 + **KODEX200 대비 1M**(아랫줄 — 다른 두 화면의 KODEX 대비 지표와 같은 자리) / 배지 보유 3-state·데이터 상태(정상 숨김) / **펼침 상세** 6개월·12개월·3년·KODEX200 대비 3M·고점 대비·점수 근거.

**`1년` 열 제거 (17→16)**: 기존 코드 주석이 스스로 "`1년` = backend twelve_month 와 동의어. 동일 값 또 표시" 라고 적고 있던 중복. 카드·표 양쪽 반영.

**⚠ 표 "크기 조절" 원인 재진단**: 착수 전 "글자·여백 키우기" 로 보고했으나, 베이스 `.market-topn-table` 은 **이미 14px·패딩 8px** 였다. 진짜 원인은 `width:100%` 표에 16열이 들어가 **열당 70px 남짓으로 눌린 것**. 가로 스크롤 컨테이너 + `min-width:1500px` 으로 폭을 확보하고 패딩·행 높이를 키웠다. **글자 크기는 변경하지 않았다.**

**지시문 외 변경 1건(승인 범위 내)**: 보유 배지를 채우려 `MarketDiscoveryView` 가 `DASH_KEY_HOLDINGS` 공유 캐시를 조회하게 했다. 신규 endpoint·N+1 없음. 미로드면 `보유 확인 불가`(미보유로 축약 안 함).

**테스트**: 이 화면은 원래 **테스트 0건**이었다. 카드 표시 계약 6건 신설(`CandidateCards.test.tsx`) — 17열 미유실·보유 3-state·데이터 상태 정상 시 미표시·onSelect·빈 상태. **표(`CandidateTable`)는 여전히 테스트 없음.**

**검증(맥북)**: tsc 0 · eslint 0 · vitest **167 passed (15 files)** (161→167) · 프론트 200. 백엔드 무변경.

**결과서**: `docs/ai_result/POC3/POC3-MARKET_DISCOVERY_CARD_CONVERSION_RESULT.md`

**다음**: 표로 남은 5개 — `HoldingsCompareView`(14열) · `OverlapTab`(11) · `AISessionsListTab`(8) · `ConstituentsTab`(6) · `EvidenceDetails`(8).

---

## 직전 작업 요약 (**손익 색 국내 관례 전환 + 배지 규칙 통일** — 코드·검증 완료 · 사용자 실화면 확인 대기 · 검증자 미착수)

## 이번 작업 요약 (손익 색 전환 + 배지 통일 — 사용자 직접 지시)

**상태**: 코드·자동 검증 **DONE** · **사용자 실화면 확인 대기**.

**손익 색**: 국내 증권사 관례(상승 빨강 / 하락 파랑)로 전환. 전용 토큰 `--pnl-up #d92d20` / `--pnl-down #1570ef` 신설(안 B). 기존처럼 `--ok`/`--danger` 를 재사용하면 **상승에 "위험" 색**을 쓰게 돼 같은 화면의 경고 표시와 의미가 섞이기 때문.

**⚠ 착수 전 보고 정정**: 색 함수가 "두 곳" 이라고 보고했으나 전수 조사 결과 **여섯 곳**이었다 — `.pnl-pos`/`.pnl-neg`(←`pnlClass`) · `directionColor` · `returnPctColor`(CandidateTable) · `returnColor`(holdings_compare) · DashboardView 인라인 2곳. 전부 토큰 참조로 통일. **앞으로 방향 색은 토큰 두 줄만 고치면 된다.**

**의도적 제외**: 방향이 아니라 **상태**를 뜻하는 색 3종 — `pctColor`(괴리율 절대값 경고) · `badgeColor`(ok/warn/danger) · `exposureColorByState`(정상/불가).

**배지 규칙 통일**: 후보 탭은 정상이면 배지를 숨기는데 보유 탭은 계속 띄우고 있었다(같은 화면 두 탭이 다른 규칙). 보유 탭을 후보 탭에 맞췄다 — **상시 참이던 `보유` 배지 제거**(정보량 0), `NAV`·`구성종목`·`근거 정상` 숨김. 이상 상태 표시는 그대로. **배지가 없으면 전부 정상**이라는 뜻이 된다.

**테스트 2건 재작성(기대값 반전 아님)**: 정상 배지가 사라져 존재 검사가 불가능해진 2건을, `NAV·구성종목 분리` 는 **구성종목만 이상인 상황**을 만들어 그 쪽만 뜨는지 보도록, `Evidence 배지` 는 **정상 시 미표시 + 부재 시 확인 불가 표시 2건**으로 나눴다. 분리 계약을 이전보다 정확히 검사한다.

**개별주 종목명 조회 — 철회**: 사용자가 "종목명은 직접 입력하면 된다. 평가금액·홀딩스 평가에서만 안 빠지면 된다"로 조건을 좁혔고, **개별주도 시세·평가·손익이 정상 산출됨을 실측 확인**(네이버 `stock/{ticker}/basic` 은 ETF·개별주 공통). 설계자 요청 철회 — 공유 보고서 §7·백로그 반영.

**검증(맥북)**: tsc 0 · eslint 0 · vitest **161 passed (14 files)** · 프론트 200. 백엔드 무변경.

**결과서**: `docs/ai_result/POC3/POC3-PNL_COLOR_AND_BADGE_CONSISTENCY_RESULT.md`

**다음**: 표로 남은 6개 화면 카드 전환 — `CandidateTable`(17열) 1순위로 목업 예정. `HoldingsCompareView`(14) · `OverlapTab`(11) · `AISessionsListTab`(8) · `ConstituentsTab`(6) · `EvidenceDetails`(8).

---

## 직전 작업 요약 (**메뉴 재편 — ML 실험 신설 · 데이터 상태/OCI 운영 상태 복원·분리 · diagnostics→개발·실험용**. 사용자 실화면 확인 완료 · 검증자 미착수)
...
**상태**: **`CLOSED` (2026-08-21)**. 아래 `PARTIAL` 잔여 3건을 실측으로 정리했다.

| 당시 미결 | 현재 |
|---|---|
| 보유 탭 배지 정리 | **완료(2026-08-16)** — `HoldingTable.tsx` 주석·코드 실측: *"정상(ok)이면 배지를 띄우지 않는다"*, 상시 참이던 `보유` 배지 제거 |
| 검증자 검증 미착수 | **대상 아님** — 사용자 확정(2026-08-19) *"UI변경이기에 검증자를 거치지 않았습니다"* |
| `확인 필요` 탭 미전환 | **미전환 유지** — 결함이 아니라 **사용자 미결정** 사항. 표가 아니라 항목 목록이라 카드 규칙을 그대로 적용할지 정해지지 않았다 |

→ **열린 `REJECTED`/`PARTIAL` 없음.** BACKLOG 정합 Step 진입 가능.
...
**목적**: Telegram PUSH 운영 정책과 PC·OCI 역할 경계를 canonical 문서에 정합화. 기존 "일 3회 자동 PUSH" 전역 제한 표현이 PUSH 종류 3개와 하루 알림 3회 제한을 혼동하게 하는 문제 해소.
...
|---|---|
| 전역 일일 PUSH 횟수 제한 | **제거** |
| Market briefing | 평일 08:00 KST 1회 정기 (유지) |
| Holdings briefing | 평일 하루 3개 평가·발송 슬롯 (오전 · 장중 · 마감) |
| Spike/Falling Alert | 조건 발생형 예외 알림 (고정 시각·횟수 없음) |
| 평가 실행 ↔ 사용자 알림 | 구분 (평가 다회 실행이라도 신호 없으면 알림 0) |
| OCI 런타임 가격 조회 | 제한적 허용 (기존 승인된 시세 출처 · Holdings 평가 재계산 · Spike 조건 재평가 · active PARAM 적용 · as-of 기록) |
| Published Evidence | read-only 유지 (OCI 임의 수정 X) |
| Runtime Evidence | Published Evidence 대체·덮어쓰기 없음 |
| 가격 조회 실패 | stale 값 발송 금지 · `sent/no_signal/duplicate/partial/failed` 구분 |
| OCI 계속 금지 | 전략·factor·threshold·ML·Holdings·주문 변경 |

**canonical 순서 (2026-07-24 정정, 이번 STEP 삽입)**:
```text
1. Telegram Push Operating Boundary Amendment v1     (현재 활성 · DONE)
...
**AC-1~AC-14 전 항목 충족**. §14 하지 않는 것 전 항목 준수 (코드·DB·Frontend·Scheduler·crontab·Telegram 발송 미변경).

**Regression/테스트**: 코드 변경 없음 → §17 에 따라 회귀 미수행. 문서 정합성 9항 자체 검증 통과.
...
## 이전 STEP 요약 (Mobile Decision Cockpit v1 — **DEFERRED_BY_USER · Unit 0 조사 완료 · Unit 1~3 미착수 · next_step_gate = LOW_FREQUENCY_TELEGRAM_PUSH_OPERATION_V1**)

최종 업데이트: 2026-07-22 (Mobile Decision Cockpit v1 — **DEFERRED_BY_USER · Unit 0 조사 완료 · Unit 1~3 미착수 · next_step_gate = LOW_FREQUENCY_TELEGRAM_PUSH_OPERATION_V1**)

## 이번 STEP 요약 (Mobile Decision Cockpit v1, DEFERRED_BY_USER)
...
**성격**: FAIL 이 아니라 사용자 운영 판단에 따른 의도적 보류. 기능·코드·DB·Frontend·Scheduler 변경 없음.

**Unit 0 조사 완료** (핵심):
- FastAPI 인증·권한 미들웨어 없음 (CORSMiddleware localhost:3000 만)
- Approve → SCP OCI outbox 실제 트리거. 판단 저장으로 자동 매핑 금지
- PENDING evidence 는 `run_id` 자체가 immutable snapshot (draft_payload 9 keys + recommendations 35 + factor_signals 6)
- DIRECT_LOOKUP evidence 는 재개 시 최소 immutable snapshot 필요 (asof/refresh_id 참조만으로는 부족)
- 판단 기록 저장 후보: `state/user_decisions/{decision_id}.json` (신규 DB 아닌 기존 JSON state 패턴 확장) — 구현 안 함
- 모바일 Web 보안 후보: Tailscale Serve · Cloudflare Tunnel · 공개 HTTPS+Basic Auth 는 후순위 — 채택 안 함

**보류 결정 사유**: 모바일 Web 을 착수하면 OCI Web 배포 + 인증 + 보안 접근 + 제한 판단 write + PC/모바일 공통 저장 + Deep link + 보안 검증 범위가 동시에 필요. PC 판단 흐름이 충분히 완성되지 않은 상태에서 이 범위를 진행하면 프로젝트 본체가 모바일 인프라·보안 작업에 다시 묶일 위험.

**재개 트리거 (4개 모두 충족 시에만)**: PC 판단 흐름 완성 · Telegram 저빈도 운영 안정화 · 모바일 부재가 다시 운영 차단 사유 · 사용자 명시 재개 결정. 재개 시 첫 후보는 **Telegram Cockpit** (모바일 Web 아님).
...
## 이전 STEP 요약 (Mobile Decision Operating Boundary Amendment v1 — **DONE · PASS · 문서 정합성 고정 · next_step_gate = MOBILE_DECISION_COCKPIT_V1**)

최종 업데이트: 2026-07-20 (Mobile Decision Operating Boundary Amendment v1 — **DONE · PASS · 문서 정합성 고정 · next_step_gate = MOBILE_DECISION_COCKPIT_V1**) ← 이후 2026-07-22 Mobile Deferred 로 gate 재변경
...
**FIX r1 (2026-07-12)**: 설계자 REJECTED 지시 후 다음 항목 해소.
- **오염 원인**: Refactor v1 회귀 test 중 `_create_approved_manual_seed_param` 이 실제 `latest_runtime_param.json` + DB active pointer 를 `isolation_test` marker 로 오염.
