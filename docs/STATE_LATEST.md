...
최종 업데이트: 2026-08-19 (**중복률 탭 카드 전환 + 구성종목 탭 식별 줄 정리** · 사용자 실화면 확인 완료 · **UI 변경이라 검증자 미대상**. 직전 08-17 A11 라운드는 `VERIFIED`)
...
K6 방어 정책(지시문 §4)"*. **조회가 아니라 저장 시점** 제한. 캐시에 ETF당 30건 저장된
과거분(`2026-06-22`, 6개)이 있어 **지금 재수집하면 10건으로 덮인다.**
