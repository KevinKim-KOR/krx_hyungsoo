...
| **exposure_ratio** | 전체 기간 중 포지션 보유일 비율 (현금 100%일 제외) | `포지션_보유일 / 전체_거래일` |
...
def objective(trial, lookbacks=[3, 6, 12]):
    ...
    scores = []
    for lb in lookbacks:
        # Step 2: params로 백테스트 실행 (trial 아님!)
        result = run_backtest(
...
def generate_windows(start_date, end_date, train_months, val_months, test_months, 
    ...
    while True:
        ...
        windows.append({
            'train': (
                snap_to_trading_day(current_start, trading_calendar),
                snap_to_trading_day(train_end - timedelta(days=1), trading_calendar)
            ),
            'val': (
                snap_to_trading_day(val_start, trading_calendar),
                snap_to_trading_day(val_end - timedelta(days=1), trading_calendar)
            ),
            'test': (
                snap_to_trading_day(test_start, trading_calendar),
                snap_to_trading_day(test_end - timedelta(days=1), trading_calendar)
            ),
        })
