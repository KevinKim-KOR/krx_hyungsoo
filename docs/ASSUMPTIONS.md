...
- **선행 조건**: 시계열 적재가 먼저. 단면 데이터로는 본 축을 다룰 수 없다.
  ...
  - VIX 일별 종가 시계열 (현재 확보 — 2026-07-03 Market Risk Reference v1 STEP 완료). FDR VIX 실측 2014-04-08 ~ 2026-07-03 / 3079 rows / `market_benchmark_daily_price` (benchmark_id='VIX'). Cboe 미사용. VIX 는 사용자 참고 evidence 로만 사용 — ML feature / 라벨 / 판단 미포함.
...
- 시장 우선 판단 후 필요한 종목만 상세 확인한다.
- 데이터의 완전성보다 사용자가 실제로 읽고 판단할 수 있는 흐름을 우선한다.
