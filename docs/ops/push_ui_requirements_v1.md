...
> 🚫 **CP-3 규정**: 
> - 모든 Action 버튼은 **티켓(Request) 생성**만 수행합니다.
> - **subprocess 호출, 엔진 직접 실행, 스크립트 자동 실행은 금지**됩니다.
> - 실제 실행은 운영자가 티켓을 확인한 후 수동으로 수행합니다.

| 버튼 | 라벨 | 동작 |
|------|------|------|
| `OPEN_DASHBOARD` | "대시보드 열기" | `/` 로 이동 |
| `REQUEST_RECONCILE` | "재조정 요청" | 티켓 생성 모달 (실행 아님) |
| `REQUEST_REPORTS` | "리포트 재생성 요청" | 티켓 생성 모달 (실행 아님) |
