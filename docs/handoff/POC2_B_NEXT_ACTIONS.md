...
작성일: 2026-05-20 / 갱신: 2026-07-09 (OCI Database Environment Remediation v1 — PARTIAL, 사용자 OCI 실행 대기)
성격: **방향을 잊지 않기 위한 앵커.** 새로운 가드 문서가 아니다. 설계 결정이
흔들릴 때 PROJECT_ORIGIN_INTENT / 시장 우선 운영 원칙과 함께 본 문서로 복귀한다.

---

## 0. 직전 STEP 결과 (OCI Database Environment Remediation v1, PARTIAL 2026-07-09)

**목적**: OCI `state/market/market_data.sqlite` 부재를 1회성 시드 반영으로 복구.

**협업 방식 (Q1 (b))**: 개발자 = PC 확인 + OCI 명령 세트 작성 + 결과 검증. 사용자 = OCI 실제 실행. 개발자 OCI 직접 접속 · SCP · SSH · rsync 실행 금지.

**PC source DB 확인 완료 (2026-07-09, Q2 (a) SHA-256)**:
- size 131,538,944 bytes / SHA-256 `f7df867d0f69fc07929b0a25a87ccdc0f235a01097299a9a522bf991614cf286` / integrity_check=ok / schema_version=12 / table_count=12.

**PARTIAL 사유**: 사용자 OCI 실행 (temp 전송 → 검증 → atomic rename → target 재검증 → Preflight 재실행) 대기.

**변경 없음** (지시문 §5): 소스 코드 · JSON · runtime · API · UI · scheduler · Telegram · `available_sources=None` 등 모두 0건.

**다음 활성 Step 게이트**:
- OCI Preflight 재실행 결과 market_data readiness=READY → DONE → `PARAM / Runtime State DB Mapping v1`.
- NOT_READY → 같은 STEP 추가 라운드.
...
**직전 활성 Step (PARTIAL 2026-07-09, 사용자 OCI 실행 대기)**: **OCI Database Environment Remediation v1**. PC source DB SHA-256 산출 완료 + OCI 명령 세트 handoff. 사용자 OCI 실행 결과 대기. 상세는 §0 참조.
...
**다음 활성 Step 게이트**: OCI Preflight 재실행 결과 market_data=READY → DONE → `PARAM / Runtime State DB Mapping v1` / NOT_READY → 같은 remediation STEP 추가 라운드.
