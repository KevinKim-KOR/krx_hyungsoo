...
작성일: 2026-05-20 / 갱신: 2026-07-09 (OCI Database Environment Remediation v1 — DONE, 3-way SHA-256 일치 · Preflight READY)
성격: **방향을 잊지 않기 위한 앵커.** 새로운 가드 문서가 아니다. 설계 결정이
흔들릴 때 PROJECT_ORIGIN_INTENT / 시장 우선 운영 원칙과 함께 본 문서로 복귀한다.

---

## 0. 직전 STEP 결과 (OCI Database Environment Remediation v1, DONE 2026-07-09)

**목적**: OCI `state/market/market_data.sqlite` 부재를 1회성 시드 반영으로 복구.

**협업 방식 (Q1 (b))**: 개발자 = PC 확인 + OCI 명령 세트 작성 + 결과 검증. 사용자 = OCI 실제 실행. 개발자 OCI 직접 접속 · SCP · SSH · rsync 실행 금지 (지시문 §7 준수).
...
- integrity_check=ok (3회) / table_count=12 (3회, table list 동일).
- OCI target 크기 = 131,538,944 bytes = PC source.

**OCI Preflight 재실행 (revision `c6967d1a`, same_revision=True)**:
- `[market_data] readiness=READY` ✅
- `single_environment_readiness=READY` ✅
- `decision_evidence` OPTIONAL_MISSING 유지 (범위 밖), runtime/staging 범위 밖 유지.

**기존 target 처리**: `absent` (Preflight `fd7ff116` 상 부재대로 실행 시점 재확인). `.remediation-backup-*` 백업 없음.

**원자적 교체**: `mv .tmp → market_data.sqlite` (POSIX rename).

**변경 없음** (지시문 §5): 소스 코드 · JSON · runtime · API · UI · scheduler · Telegram · `available_sources=None` · `decision_evidence.sqlite` · `three_push_runtime_probe_latest.json` 등 모두 0건.

**다음 활성 Step (확정)**: **`PARAM / Runtime State DB Mapping v1`** (설계자 확정 세션).
...
**직전 활성 Step (DONE 2026-07-09, 3-way SHA-256 일치 · Preflight READY)**: **OCI Database Environment Remediation v1**. OCI `market_data.sqlite` 시드 반영 완료 · single_environment_readiness=READY · revision `c6967d1a`. 상세는 §0 참조.

**이전 활성 Step (DONE 2026-07-08, 사전점검)**: **OCI Database Preflight v1**. PC · OCI 교차 실측 완료 · overall NOT_READY (market_data 부재). 상세는 §0-prev 참조.

**이전 활성 Step (DONE 2026-07-07, 사전 감사)**: **OCI Active Data Boundary Audit v1**.
