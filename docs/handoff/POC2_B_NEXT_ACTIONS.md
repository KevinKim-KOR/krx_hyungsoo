...
작성일: 2026-05-20 / 갱신: 2026-07-13 (Runtime Evidence DB Connection v1 — PARTIAL, PC DONE · OCI 실행 대기)
성격: **방향을 잊지 않기 위한 앵커.** 새로운 가드 문서가 아니다. 설계 결정이
흔들릴 때 PROJECT_ORIGIN_INTENT / 시장 우선 운영 원칙과 함께 본 문서로 복귀한다.

---

## 0. 직전 STEP 결과 (Runtime Evidence DB Connection v1, PARTIAL 2026-07-13 · PC DONE)
...
**PC 결과 (FIX r1 최종)**: backend **845 passed** (827 → 838 → 845, 순증 18). 0 fail. 실제 state 3종 (runtime_state.sqlite / latest JSON / market_data.sqlite) pytest 전·후 완전 불변.

**FIX r1 정정 항목** (검증자 REJECTED 대응):
- `contentful_fact_count` push_kind 별로 실제 사용하는 source 만 집계 (holdings/spike 잘못된 Market Discovery 합산 제거).
- Spike 는 `compute_topn` 미호출 (B-6 불필요 계산 제거).
- `source_statuses` 라벨 `not_applicable` → `unavailable` (고정 unavailable source).
- Market Discovery preview 문장에 as-of 명시 (기준일 조건 강화).
- Holdings loader broad `Exception` → `(HoldingsValidationError, json.JSONDecodeError, ValueError)` 만 격리, 프로그래머 오류 propagate (B-1).
- Runner dry-run 자동 회귀 test 4 신규 (§15 커버 · 검증자 A-1 지적).
- Composer test 3 신규 (spike contentful=0 assert, source_statuses unavailable assert, broad exception propagate).
- 실제 PC 운영 경로 Composer 직접 실측 값 CONCLUSION §4 반영.

**dry-run 계약 유지**: runtime status DB write / history JSONL append YES, Telegram 호출 · sent registry write NO.

**PARTIAL 사유**: OCI 3-PUSH dry-run 사용자 실행 결과 대기 (§16 · §18).
...
**직전 활성 Step (PARTIAL 2026-07-13, PC DONE + FIX r1 · OCI 실행 대기)**: **Runtime Evidence DB Connection v1**. Composer 신설 · market_discovery/holdings/nav 연결 · runner + diagnosis 정합화 · FIX r1 (contentful count · source_statuses · B-1 · B-6 · runner dry-run test) 반영 · 845 passed. 상세는 §0 참조.
...
**다음 활성 Step (조건부)**: OCI 결과에 따라 `Telegram Contentful Controlled Send v1` 또는 `OCI Evidence Publication / Missing Source Connection`.
