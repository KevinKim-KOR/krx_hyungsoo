...
작성일: 2026-05-20 / 갱신: 2026-07-12 (Runtime State Store Refactor & Test Isolation v1 — VERIFIED, commit `f43e5565`)
성격: **방향을 잊지 않기 위한 앵커.** 새로운 가드 문서가 아니다. 설계 결정이
흔들릴 때 PROJECT_ORIGIN_INTENT / 시장 우선 운영 원칙과 함께 본 문서로 복귀한다.

---

## 0. 직전 STEP 결과 (Runtime State Store Refactor & Test Isolation v1, DONE 2026-07-10)
...
**직전 활성 Step (VERIFIED 2026-07-12, commit `f43e5565`)**: **Runtime State Store Refactor & Test Isolation v1**. `runtime_state_store.py` 삭제 · 4 store 분리 · test isolation fixture · verify CLI 판정 강화 · 회귀 test 4. Cutover v1 최종 PASS 자격 확정. 상세는 §0 참조.

**이전 활성 Step (DONE 2026-07-09, PC + OCI verify READY)**: **PARAM / Runtime State DB Cutover v1**. commit `13fc8a09`.

**이전 활성 Step (DONE 2026-07-09, 매핑 계약)**: **PARAM / Runtime State DB Mapping v1**. commit `8a7a7ccc`.
