...
작성일: 2026-05-20 / 갱신: 2026-07-08 (OCI Database Preflight v1 — PARTIAL, OCI 실측 대기)
성격: **방향을 잊지 않기 위한 앵커.** 새로운 가드 문서가 아니다. 설계 결정이
흔들릴 때 PROJECT_ORIGIN_INTENT / 시장 우선 운영 원칙과 함께 본 문서로 복귀한다.

---

## 0. 직전 STEP 결과 (OCI Database Preflight v1, PARTIAL 2026-07-08)

**목적**: OCI SQLite 운영 전환 사전점검. read-only 실측만. DB / JSON / runtime / API / UI / scheduler / transfer 변경 0건 (지시문 §4 · §5 준수).

**PC 실측 (revision `13ced48e`)**:
- `state/market/market_data.sqlite` READY (integrity=ok, 12 tables).
- `state/decision/decision_evidence.sqlite` READY (OPTIONAL, integrity=ok).
- runtime paths: PC 로컬 2개 존재 + audit 근거 인용 → `confirmed_from_local_and_prior_audit`.
- staging: env 부재 + remote 실제 상태 미확인 → `unconfirmed_from_audit` (Q2 확정본 준수).
- PC single_environment_readiness = READY.

**PARTIAL 사유** (§7.2): OCI 실측 미완료 → overall 판정 불가. 사용자 OCI 실행 대기.
...
- NOT_READY → `OCI Database Environment Remediation v1`.
- PARTIAL → 같은 Preflight v1 이어서 완료.
...
**직전 활성 Step (PARTIAL 2026-07-08, 사전점검 · OCI 실측 대기)**: **OCI Database Preflight v1**. PC 실측 READY (revision `13ced48e`, market_data / decision_evidence 모두 integrity=ok). 신규 CLI + tests 19 케이스 (backend 809 passed, FIX r1: sanitised failure contract 회귀 3건 추가). 상세는 §0 참조.

**이전 활성 Step (DONE 2026-07-07, 사전 감사)**: **OCI Active Data Boundary Audit v1**. 활성 데이터 boundary 전수 감사 (216 file). 상세는 §0-prev 참조.

**이전 이전 활성 Step (DONE 2026-07-07, 원인 진단)**: **PUSH Content Gap Diagnosis v1**.

**다음 활성 Step 게이트**: OCI 실측 완료 후 재판정. READY → PARAM/Runtime State DB Mapping v1 / NOT_READY → OCI Database Environment Remediation v1 / PARTIAL → 같은 Preflight v1 이어서 완료.
