...
**PC 결과 (FIX r2 최종)**: backend **848 passed** (827 → 838 → 845 → 848, 순증 21). 0 fail. 실제 state 3종 (runtime_state.sqlite / latest JSON / market_data.sqlite) pytest 전·후 완전 불변.
...
**FIX r2 정정 항목** (검증자 재지적 대응):
- A-1: Holdings `market_asof` 부재 시 unavailable 처리 (지시문 §8 "as-of 없으면 available X" 준수). Holdings 각 evidence 문장에 `(YYYY-MM-DD 기준)` 명시.
- B-1: `ValueError` 를 catch 대상에서 제외 → `(HoldingsValidationError, json.JSONDecodeError)` 만 데이터 오류로 격리. 프로그래머 `ValueError` 는 propagate.
- A-3: Holdings diag 에 `asof` 키 저장 (source_asof 조립 계약과 통일). Holdings source_asof 가 정확히 반영 확인.
- A-3: STATE_LATEST L25 테스트 수 11 → 17 정정. Composer 라인수 504 → 522 정정.
- B-6: Holdings/Spike dry-run test 에 Telegram spy 미호출 + `sent_registry` 불변 직접 assert 추가.
- Composer test 3 신규 (ValueError propagate · market_asof unavailable · source_asof 확인).
...
**직전 활성 Step (PARTIAL 2026-07-13, PC DONE + FIX r2 · OCI 실행 대기)**: **Runtime Evidence DB Connection v1**. Composer 신설 · market_discovery/holdings/nav 연결 · runner + diagnosis 정합화 · FIX r1/r2 반영 · 848 passed. 상세는 §0 참조.
