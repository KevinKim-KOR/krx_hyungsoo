...
작성일: 2026-05-20 / 갱신: 2026-07-09 (PARAM / Runtime State DB Mapping v1 — DONE)
성격: **방향을 잊지 않기 위한 앵커.** 새로운 가드 문서가 아니다. 설계 결정이
흔들릴 때 PROJECT_ORIGIN_INTENT / 시장 우선 운영 원칙과 함께 본 문서로 복귀한다.

---

## 0. 직전 STEP 결과 (PARAM / Runtime State DB Mapping v1, DONE 2026-07-09)
...
**직전 활성 Step (DONE 2026-07-09, 매핑 계약 확정)**: **PARAM / Runtime State DB Mapping v1**. code_contract + OCI observed 일치, `runtime_state.sqlite` 신설 예정 + 5 table 후보 · JSON→DB 매핑표 확정. 상세는 §0 참조.

**이전 활성 Step (DONE 2026-07-09, 시드 반영)**: **OCI Database Environment Remediation v1**. OCI `market_data.sqlite` READY · commit `22d29193`. 상세는 §0-prev 참조.

**이전 활성 Step (DONE 2026-07-08, 사전점검)**: **OCI Database Preflight v1**.
