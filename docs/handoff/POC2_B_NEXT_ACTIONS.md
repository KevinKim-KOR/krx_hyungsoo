...
작성일: 2026-05-20 / 갱신: 2026-07-09 (PARAM / Runtime State DB Cutover v1 — DONE, PC + OCI verify READY)
성격: **방향을 잊지 않기 위한 앵커.** 새로운 가드 문서가 아니다. 설계 결정이
흔들릴 때 PROJECT_ORIGIN_INTENT / 시장 우선 운영 원칙과 함께 본 문서로 복귀한다.

---

## 0. 직전 STEP 결과 (PARAM / Runtime State DB Cutover v1, DONE 2026-07-09)
...
**직전 활성 Step (DONE 2026-07-09, PC + OCI verify READY)**: **PARAM / Runtime State DB Cutover v1**. `runtime_state.sqlite` PC/OCI 양쪽 seed 완료 · 5 table · same_revision=True (`16956f95`). PC↔OCI PARAM hash 불일치는 Q9 operational warning + §12.2 BACKLOG 이월. 상세는 §0 참조.
