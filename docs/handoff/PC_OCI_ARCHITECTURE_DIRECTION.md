...
## OCI 경계 정정 (2026-07-20, Mobile Decision Operating Boundary Amendment v1)

기존 "OCI read-only" 표현을 다음처럼 정정한다.

```text
OCI 의 Market · Holdings · Universe · ML evidence 는 published read-only 다.

모바일 · PC 에서 생성되는 사용자 투자 판단 기록에 한해
제한된 command write 를 허용할 수 있다.
