...
> - **현재 운영 상태 (2026-07-26)**: Low-Frequency Telegram Push Operation v1 = **DONE**. Market·Holdings·Spike 전부 OCI ACTIVE. OCI 07:20 데이터 배치 (증분 시세 갱신 + SQLite Universe artifact + freshness) 후 Market 08:00 / Holdings 3슬롯 / Spike 7 tick 발송. OCI 실측 완료 (Spike sent · Telegram 실 수신). next_step_gate = **FIRST_REAL_DECISION_CYCLE_V1**.
...
> - 상세: MASTER_PLAN "Telegram 저빈도 운영 우선 앵커 (2026-07-22)" + "Telegram PUSH 운영 경계 정정 (2026-07-24)" 섹션 · `docs/handoff/POC2/POC2_TELEGRAM_PUSH_OPERATING_BOUNDARY_AMENDMENT_V1_CONCLUSION.md`.
...
**목적**: 신규 앵커 `docs/handoff/POC2/POC2_MOBILE_DECISION_OPERATING_SEQUENCE_ANCHOR.md` (사용자 확정, 2026-07-20) 를 canonical 문서에 반영하고 기존 두 충돌 해소:
...
상세: `docs/handoff/POC2/POC2_HOLDINGS_EVIDENCE_OCI_PUBLICATION_V1_CONCLUSION.md` §16 (Cleanup / FIX r7 Closeout).
...
- `scripts/run_push_content_gap_diagnosis.py`, `tests/test_push_content_gap_diagnosis.py`, `docs/handoff/POC2/POC2_PUSH_CONTENT_GAP_DIAGNOSIS_V1_CONCLUSION.md`.
...
**신규 파일** (B-2/B-3 준수 위해 v2 코드는 책임별 3 모듈로 분리): `app/market_flow_v2_predictor.py` (137줄), `app/market_flow_v2_diagnostics.py` (299줄), `app/market_flow_v2_model_comparison.py` (298줄), `scripts/run_market_flow_v2_model_comparison.py`, `tests/test_market_flow_v2_model_comparison.py`, `docs/handoff/POC2/POC2_MARKET_FLOW_ML_V2_DATA_VALIDITY_MODEL_COMPARISON_CONCLUSION.md`.
...
상세: `docs/handoff/POC2/POC2_MARKET_FIRST_OPERATING_DIRECTION.md`.
...
- `docs/handoff/POC2/POC2_BACKLOG_AUDIT_CONCLUSION.md` — 신규. 분류표 / 사용자 모호 항목
...
- **신규 산출물 3종**: `scripts/run_three_push_sync_task.ps1` (PS wrapper) /
  ...
  `docs/handoff/POC2/POC2_OCI_THREE_PUSH_OPERATION_REGISTRATION_CONCLUSION.md` (conclusion).
...
- 인증키 활성화 대기 동안에는 ETF Exposure 메뉴를 사용 불가 상태로 인벤토리
  명시 (`docs/handoff/POC2/POC2_FEATURE_INVENTORY.md` §2.10~12).
