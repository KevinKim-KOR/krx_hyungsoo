...
| `docs/handoff/POC2/POC2_B_NEXT_ACTIONS.md` | §0 본 Step 결과 + 다음 분기 후보 4건 |
| `docs/handoff/POC2/POC2_FEATURE_INVENTORY.md` | §2.27 신규 (PARAM runtime 정식 항목) + §2.26 격하 표기 |
...
| `docs/handoff/POC2/POC2_PARAM_HANDOFF_OCI_RUNTIME_CONCLUSION.md` | 신규 (본 문서) |
...
**1차 commit 시점**: 533 passed + 1 failed (`test_generate_spike_alert_via_unified_endpoint`, 기존 회귀).
**FIX r1 후**: **578 passed** + 1 동일 기존 회귀 (직전 Step 533 + 본 Step 신규 45 테스트). 신규 테스트 전부 PASS — 중첩 forbidden key rejection / 대소문자 무관 / 파일 IO 손상 등 가드 정합성 직접 검증.
...
- **FIX r1 (검증자 NOTES 반영, 2026-06-19)**:
  - A-1 / B-1: `validate_param_dict` 와 `verify_three_push_param_oci.py` 의 forbidden key 검사를 **top-level → 재귀 (dict/list 중첩 포함)** 로 강화. 대소문자 무관 매칭. 오류 메시지에 위반 경로 (`runtime_policy.nested.message_text` 등) 표시.
  - B-6: PARAM 경로 전용 테스트 신규 2개 추가 (`tests/test_three_push_runtime_param.py` 25 케이스, `tests/test_three_push_runtime_message_builder.py` 14 케이스 — 총 39+ 신규 테스트). 중첩 forbidden key 거부 / 대소문자 무관 / from_dict 예외 / file IO 손상 등 포괄. OCI verifier 도 OCI 측에서 중첩 거부 + rc=1 실측 확인.
  - A-3: `OCI_THREE_PUSH_CRONTAB_TEMPLATE.md` §1 전제 조건을 PARAM runtime 기준으로 교체 + §1-fallback 으로 package 경로 분리. `PC_THREE_PUSH_SYNC_TASKSCHEDULER.md` 의 "stdout만 append" 표기를 "stdout/stderr 모두 append (`*>`)" 로 정정.
