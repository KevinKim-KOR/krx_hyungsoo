...
작성일: 2026-05-27 / 갱신: 2026-07-07 (PUSH Content Gap Diagnosis v1 — DONE, Closeout 완료)
...
**직전 Step (DONE 2026-07-05, Walk-forward)**: Market Flow ML Walk-forward Lookback v1 — Ridge baseline v1 의 과거 반복 성능 evidence 산출 (예측 110 건, 2017-07-06 ~ 2026-06-01). Ridge directional_accuracy 0.5273 vs simple baseline 0.5727. 상세: `docs/handoff/POC2/POC2_MARKET_FLOW_WALK_FORWARD_LOOKBACK_V1_CONCLUSION.md`.

**직전 Step (DONE 2026-07-05, Baseline Closeout)**: Market Flow ML Dataset + Baseline v1 Closeout — scikit-learn 1.9.0 승인/선언, KOSPI 역사 시계열 CLI 보강 (2870 행), real SQLite baseline 실측 (split 1756/572/592, latest_inference=ok). 상세: `docs/handoff/POC2/POC2_MARKET_FLOW_ML_DATASET_BASELINE_V1_CONCLUSION.md`.

본 인벤토리의 기능들은 **시장 우선 운영 원칙** (`docs/handoff/POC2/POC2_MARKET_FIRST_OPERATING_DIRECTION.md`) 하에서 역할이 고정된다:
...
### 2.42 PUSH Content Gap Diagnosis v1 (2026-07-07, DONE — Closeout 완료)
...
| 사용 가능 여부 | **사용 가능** (DONE, 2026-07-07). |
...
| PC · OCI 교차 실측 (양쪽 commit `89f7cd31`) | 세 PUSH 모두 `primary_root_cause=RUNTIME_CONFIGURATION_GAP`, `exact_reason_code=runtime_available_sources_not_supplied`, `selection_result_count=0`, `content_generation_status=data_insufficient`. PARAM 정상 로드 · 세 push_kind 활성. |
...
| 개별 환경 artifact `observation_status` | 원본 유지 (설계자 지시). 교차 비교 완료 사실은 Closeout 문서 / 완료 보고에서 확정. |
| 신규 파일 | FIX r2 로 core 를 책임별 4 모듈로 분리 (B-2/B-3 해소): `app/push_content_gap_diagnosis_requirements.py` (195줄, 상수 + readiness) / `app/push_content_gap_diagnosis_reproducers.py` (152줄, PARAM · package 재현) / `app/push_content_gap_diagnosis_classifier.py` (171줄, 원인 분류 + MIXED 분기 신규) / `app/push_content_gap_diagnosis.py` (210줄, main runner + artifact writer, 얇게 유지). + `scripts/run_push_content_gap_diagnosis.py`, `tests/test_push_content_gap_diagnosis.py`, `docs/handoff/POC2/POC2_PUSH_CONTENT_GAP_DIAGNOSIS_V1_CONCLUSION.md`. |
...
| 다음 조치 | 다음 STEP 유형 `OCI_RUNTIME_CONFIGURATION_CLOSEOUT` (정식 설계명은 별도 설계 세션). |
...
| 신규 파일 | `app/market_flow_v2_predictor.py` (137 줄, feature 상수 + 3모델 예측) / `app/market_flow_v2_diagnostics.py` (299 줄, target·coverage·quartile 진단) / `app/market_flow_v2_model_comparison.py` (298 줄, main runner + artifact writer). B-2/B-3 준수 위해 책임별 3 모듈 분리. + `scripts/run_market_flow_v2_model_comparison.py`, `tests/test_market_flow_v2_model_comparison.py`, `docs/handoff/POC2/POC2_MARKET_FLOW_ML_V2_DATA_VALIDITY_MODEL_COMPARISON_CONCLUSION.md`. |
...
| 신규 파일 | `app/market_flow_walk_forward.py`, `scripts/run_market_flow_walk_forward.py`, `tests/test_market_flow_walk_forward.py`, `docs/handoff/POC2/POC2_MARKET_FLOW_WALK_FORWARD_LOOKBACK_V1_CONCLUSION.md`. |
...
- 다음 STEP 후보: `docs/handoff/POC2/POC2_B_NEXT_ACTIONS.md`
