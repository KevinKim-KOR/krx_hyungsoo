...
- **독자**: 다음 세션 개발자 · 설계자
- **상태**: 검증자 **`VERIFIED`**(r10) · **사용자 preview 확인 대기** · **OCI 미배포**
...
| # | 항목 | 내용 |
|---|---|---|
| 1 | **사용자 preview 확인** | 아직 없다. 이 확인 전까지 `CLOSED` 아님 |
| 2 | **OCI 배포** | 저장소에만 반영. **배포 전까지 실제 발송 본문은 이전 형식** |
| 3 | **`DEF-PRICE-EQUITY-REFRESH`** | **시한부.** 보유 개별주 3종 종가가 2026-08-12 에 멈춤. 지금은 기준일(2026-08-07) 종가가 있어 정상 계산되나 **약 4 거래일** 뒤 기준일이 단절 시점을 넘기면 20일 관찰 불가 |
...
1. 사용자 preview 형식 확인 → 결과서·STATE 를 `CLOSED` 로
2. OCI 배포 → `PROGRAM_TRUTH` 프로세스 **C-1** 의 "미배포" 표시 제거
3. `DEF-PRICE-EQUITY-REFRESH` 처리 여부 (설계자 판단 · 유예 4 거래일)
