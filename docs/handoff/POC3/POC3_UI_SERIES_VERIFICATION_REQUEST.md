...
| A9 | **운영 코드 수정 제외** | (검증자 REJECTED 후) 개발자가 운영 `_write_status` 비원자성 수정 영향도를 보고하자 사용자가 지시 | **"운영은 다시 제외입니다. 이것부터 다시입니다."** (2026-08-17) |
| A10 | **운영 수정 영향도 질의** | 사용자: "운영에도 문제가 된다는데 운영꺼까지 수정한다면 영향도가 클까요?" → 개발자 답변: 파일 1개·함수 1개(`_write_status`)·약 4줄, 호출부 영향 없음 | **답변 후 A9 로 "운영 제외" 지시** |

> **A-2 정정 (2026-08-17 재검증)**: 최초 이 표에는 **A9 가 없었다.** 그 상태에서 결과서에
> "운영 수정은 사용자 지시로 제외" 라고 적어, **승인 기록에 없는 근거를 인용**했다.
> 검증자 지적으로 위 두 줄을 추가한다. 지시 자체는 실재했으나 **기록되지 않은 지시를
> 근거로 쓴 것은 보고 정확성 위반**이다.

| **A11** | **A9 대체 승인 — 운영 최소 수정 허용** | 검증자 2차 REJECTED 후 개발자가 승인 충돌(A3 원인 해소 vs A9 운영 제외)을 보고 | **"운영 수정 제외였던 A9를 철회하고, 검증자가 재현한 상태 파일 경쟁 조건의 최소 운영 수정을 허용한다. … `_write_status()` 를 같은 디렉터리 임시 파일 완성 기록 후 단일 `os.replace` 로 교체하도록 수정한다. active 상태 파일 1개 정본, 기존 상태 스키마·읽기 계약·API·DB·fail-loud 계약은 유지한다. 기존 테스트 안정화 수정은 보존하고, 경쟁 구간을 의도적으로 재현하는 운영 회귀 테스트를 추가한다. 검증자 재검증 전에는 다음 UI 작업으로 진행하지 않는다."** (2026-08-17) |

> **A9 → A11 로 대체됨.** A9(운영 제외)는 철회됐다. A3(원인 잡고 가기)와의 충돌이 해소됐다.

> **재검증 결과 (2026-08-17): `VERIFIED`.** A-1~A-4 · B-1~B-6 전 항목 통과.
> 경쟁 전용 테스트 5건 ×10회 **50/50**, 백엔드 **1,147 passed**, 프론트 **167 passed**,
> 위험 **NONE** · 범위 폭주 **NONE**. A11 의 "다음 UI 작업 보류" 조건 **해제**.

**A1·A3·A11 이 이번 판정의 핵심 승인 근거**다. A1 은 백엔드 응답 필드 추가,
A3·A11 은 그로 인해 드러난 운영 경쟁 조건의 해소 방침이다.
