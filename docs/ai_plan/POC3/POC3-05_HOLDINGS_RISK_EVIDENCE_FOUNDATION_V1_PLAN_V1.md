...
### 1.3 급락(Falling) 신호 — 매칭 범위 [확인 · Q1 확정]
- Falling source = `lib/api/universeMomentum.ts` `falling_candidate`(단일 `UniverseTopCandidate | null`) + `falling_threshold_pct`. UniverseRefreshPanel 이 표시. **universe 전체에서 급락 후보 "1건"** 이지 보유 종목별 신호가 아님.
- holdings evidence 의 `topn_match` 는 **상승 topn 후보 매칭**(Q2: 이번 Step 미사용).
- **Q1 확정(a)**: "기존 주의 신호(급락) 일치" = **보유 ticker ∩ `falling_candidate.ticker`** (exact match, 최대 1종목). 보유별 급락 신규 계산 안 함. `falling_candidate` 단일 한계 인정 — 이번 Step 중심은 5일·20일·시장 대비·평가 비중이고 급락 일치는 보조 근거.
...
- 요약(기준일·coverage·자료 확인 필요) + 빠른 보기(전체 / 기존 주의 신호 / 자료 확인 필요) + 고밀도 표: ETF명·ticker · **평가금액·평가 비중(`market_weight_pct`)·손익률**(enriched) · **5일·20일·KODEX200 대비 20일**(evidence short_term_momentum) · 주의 신호 일치 · 기준일 · 데이터 상태. `buy_weight_pct` 제외(Q6).
...
- `오늘의 투자 점검`의 "내가 가진 ETF(개발 중)" 실제 구현: 기존 주의 신호 일치(Q1 3-state 분기) + 최근 5일 낮은 최대 3건(중복 ticker 제거 후, Q5·Q7) + 자료 확인 필요 건수 + 기준일·coverage + `보유 ETF 확인 근거 보기`(Holdings 이동).
- Workbench 는 동일 ticker·snapshot 이면 같은 값·상태 표시(별도 계산 없음).

### 테스트
- coverage/자료확인필요 분리(Q4 기준) · 5일 정렬(로컬·유효분) · Falling exact match 3-state(성공+일치 / 성공+미일치·후보없음 / 확인불가 — unavailable 시 0건 표시 금지) · unavailable 0/정상 위장 금지 · 금지어(위험/고위험/손절/매도/청산/BUY/SELL) 비노출 · N+1 없음(2 endpoint 단일 조회) · ticker 통합 · 3화면 동일 값.
...
| 질문 | 확정 판단 |
|---|---|
| Q1 | **(a) exact match만.** `보유 ticker ∩ falling_candidate.ticker`, 최대 1종목. 보유별 급락 신규 계산 안 함. |
...
| 기존 주의 신호(급락) | `falling_candidate.ticker` exact match (보유 ∩, 최대 1) | DIRECT (3-state) |
...
### 3-B. Q1 급락 신호 3-state 분기 (섞지 않음)
- **source 성공 + falling_candidate 가 보유 ticker 와 일치** → `기존 주의 신호와 일치` (해당 ticker 표시).
- **source 성공 + 후보 없음(null) 또는 미일치** → `현재 제공된 주의 후보 없음` / `보유 종목과 일치 없음`.
- **source unavailable · not_loaded · 실패** → `주의 신호 확인 불가`. **이 경우에만 일치 건수를 `0건`으로 표시 금지.** ("후보 없음" ≠ "조회 못 함".)
...
착수 전 소스 미수정. BLOCKED 아님(핵심 Holdings evidence 단일 조회 완비). Falling 은 source 존재 → UNAVAILABLE 아님(3-state 처리).
