...
- **개정 이력**
  ...
  | **V1.6** (2026-09-10) | **검증자 `VERIFIED_WITH_NOTES`** (커밋 `988b0494`). fail-closed 계약을 코드에 연결 · 결과서 실측값 정정 · 계약 테스트 15건. **`02B-2` 이월 4건**(§18) |

- **상태**: **`OPS-02B-1` 검증 통과** (`VERIFIED_WITH_NOTES`, 커밋 `988b0494`).
  결론 **§14** · 이월 **§18**. `02B-2` 는 설계자 최종 Gate 확정 후. `OPS-02B-2` 는 `02B-1` 판정 이후
...
| # | 항목 | 근거 |
|---|---|---|
| 1 | **API 에만 있는 신규 ticker 감지** — `select_index_universe` 가 CSV 를 순회해 API 쪽 신규 종목을 보지 못한다. 일별 API 가 CSV 보다 최신이 되는 운영 단계에서 **"공식 CSV 교체 필요 상태"** 로 감지해야 한다. 현재 snapshot 에는 사례 없음 | 검증자 `VERIFIED_WITH_NOTES` B-6 |
| 2 | 정적 메타데이터 **자동 갱신 경로** 결정 (현재 수동 다운로드) | PLAN §7.2 |
| 3 | `DEF-ETF-DAILY-PRICE-BASIS-CONSISTENCY` 본조사 — 2026-04-17→04-20 역방향 계단 · source 중첩 구간 | §14.4 |
| 4 | 분배락 날짜 확보 시 무조정가 **민감도 보고** | §2.2 한계 |

---

## 19. 진행 상태와 다음 순서

`OPS-02B-1` Gate 실행 완료 · 설계자 최종 판정 반영 완료(V1.4).
...
| 6 | `VERIFIED` 후 설계자 최종 Gate 확정 · HANDOFF | **다음** |
| 7 | 새 설계자 세션에서 `02B-2` 메시지·억제·목업 설계 | — |
