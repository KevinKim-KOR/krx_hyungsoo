...
- **상태**: 구현·실행 완료 → **검증자 r1 `REJECTED`** → **r2 정정 반영**(2026-08-25).
  완료조건 11개 중 **9개 실측 충족**, 2개(UI 표시·정렬)는 **검증자 직접 확인 통과 ·
  사용자 최종 확인 대기**. r1 지적 4건과 조치는 **§10**.
...
```
재실행 전
  etf_ml_feature_daily       rows=1,376,524  sha256=da69c6ecdf18dd03a86c848439178628
  market_risk_feature_daily  rows=    3,013  sha256=41bd8853df56076d37a208f4d9c4dd6d
...
| # | 항목 |
|---|---|
| 1 | **파생 DB 1,985행 삭제**(§3.4) — 사후 보고만 했고 **승인 미확인**. 계약 정렬 목적이며 재실행으로 복원 가능(멱등) |
| 2 | 완료조건 8·9 **사용자 최종 확인** — 검증자 직접 확인은 통과했다 |
