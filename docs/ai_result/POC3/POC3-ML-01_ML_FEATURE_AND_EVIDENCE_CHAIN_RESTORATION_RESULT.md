...
- **상태**: **`VERIFIED`** (검증자 2026-08-25). **완료조건 11개 전부 충족.**
  - 경과: 구현 → r1 `REJECTED` → 정정 → r2 `REJECTED`(승인 기록 모순) → 정정 → **`VERIFIED`**.
  - **8·9(UI 표시·정렬)**: 검증자 기술 확인 **통과** + **사용자 실화면 확인 완료**.
  - **파생 DB 1,985행 삭제**: **사용자 승인 완료**(§10.5 원문).
  - r1 지적 4건 → **§10.1~10.4** · r2 지적(승인 기록·해시 표기) → **§10.5 · §4.2**.
  - **`DEF-ML-CHAIN` 해소** — `docs/backlog/BACKLOG.md` §현재 결함에서 제거.
...
| 8. UI 3종 표시 | **DONE** — 검증자 확인 + **사용자 실화면 확인 완료** |
| 9. 고점대비 정렬 동작 | **DONE** — 클릭 전후 후보 순서가 실제 값 기준으로 변경됨 |
...
### 7.1 UI 확인 2건 — **해소** (완료조건 8·9)

**검증자 직접 확인 + 사용자 실화면 확인 모두 완료**(2026-08-25).

검증자 확인 내역:

- 카드 상세 — 참고점수 `71.5` · 점수 근거 3건 · 고점 대비 `+0.00%` 표시
- 표 — 참고점수 · 고점 대비 · 점수 근거 열에 실제 값 표시
- 보유 비교 상세 — 동일 3개 항목 표시
- `ETF 비교하기` 에서 **`고점대비` 클릭 전후 후보 순서가 실제 값 기준으로 변경**
- 값이 없는 `0223A0` 은 점수·고점 대비를 `—` 로 표시 — **`0` 으로 위장하지 않음**
...
| 2 | **완료조건 8·9 실화면 확인**(§7.1) | **확인 완료 (2026-08-25)** |
...
| 2 | 완료조건 8·9 UI 표시·정렬 | **확인 완료 (2026-08-25)** — 검증자 + 사용자 실화면 |
...
## 11) 최종 판정 — `VERIFIED` (2026-08-25)

검증자 판정 **`VERIFIED`**. A-1~A-4 · B-1~B-6 전 항목 통과 · 위험 **NONE** · 범위 폭주 **NONE**.

**완료조건 11개 전부 충족.** 파생 DB 삭제 승인 · 기술 검증 · 사용자 실화면 확인까지 끝났다.
...
| `market_risk_feature_daily` | **3,013행** · 동일 기간 |
| `relative_upside_score_latest.json` | **1,157종목** · `asof 2026-08-20` |
| ML job | **3단계 success** |
| sanity | `warn` · **errors 0** |
| baseline v0 | `status=ok` · **누수 없음** |
| readiness | **7축 전부 `available`** |
| 화면 3종 | 참고점수 · 사유 · 고점 대비 **표시 · 정렬 동작** |

### 커밋 이력

| 커밋 | 내용 |
|---|---|
| `c95be6f0` | DESIGN V1 + PLAN V1 기준점 (`PLAN PASS`) |
| `f21143e7` | 구현 — B-2 + 회귀 4건 + 결과서 |
| `98e09434` | r1 정정 — 회귀 자체 격리 · 재실행 계약 · 수치 |
| `824339d2` | r2 정정 — 승인 기록 일치 · sha256 전체 표기 |
