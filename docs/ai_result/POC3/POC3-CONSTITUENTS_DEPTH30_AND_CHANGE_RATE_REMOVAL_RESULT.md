...
- **상태**: **`VERIFIED`** (검증자 재검증 2026-08-20). r1·r2 `REJECTED` → r3 에서 해소. 검증자 대상 —
...
## 13) 재검증 `VERIFIED` + 실화면 확인 (2026-08-20)

### 13.1 검증자 판정

r3 로 **`VERIFIED`**. r1(기능)·r2(보고 정확성) 지적이 모두 해소됐다.

### 13.2 실화면 — 수집 실행 결과 실측

사용자가 구성종목 수집을 실행했다. **깊이 30 요청이 실제로 반영됐다.**

```
asof 2026-08-20 · ETF 10개 · 총 180행 · 최대 rank 30
```
...
| `469070` | **30** |
| `0223A0` | 28 |
| `0221Z0` · `228790` · `470310` | 19 |
| `0148J0` · `0177X0` | 16 |
| `0008T0` · `0155N0` · `364970` | 11 |

**30건이 채워진 ETF 는 1개뿐**이고 나머지는 11~28건이다. 이것은 결함이 아니라
**해당 ETF 의 구성종목이 그만큼뿐**이라는 뜻이다(§3.2 의 "source 고갈" 케이스).
설계 확정의 *"30건 미만이면 실제 확보된 건수만 표시"* 가 그대로 적용된다.

API 응답도 같다.

```
GET /market/constituents/analysis?tickers=469070,0223A0,0008T0&top_k=30
asof 2026-08-20 · top_k 30 · overlap_top_k 10
  469070 → 30건 / 0223A0 → 28건 / 0008T0 → 11건
```

**`overlap_top_k = 10`** 이 응답에 그대로 나온다 — 중복률 기준은 표시 깊이와 분리돼 있다.

### 13.3 화면에 10건으로 보인 건 브라우저 캐시

사용자 화면에는 여전히 10건으로 보였다. **데이터·API 문제가 아니다** — 위 실측대로
DB 와 API 는 30건을 준다. Next.js dev 서버가 갱신된 JS 번들을 내보내도 Safari 가
이전 번들을 재사용하면 옛 화면이 그대로 그려진다. **강력 새로고침(⌘⇧R)** 으로 해소된다.
