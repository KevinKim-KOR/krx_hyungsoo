...
* 문서 종류: 개발 결과서 (검증자 입력)
* 작성일: 2026-07-29 (초안) / r1~r8 / r9 / **r10 검증자 REJECTED 수정 (AC-7 툴팁·보고정확성·B-1·테스트)**
* 상태: **IMPLEMENTED_AWAITING_VERIFICATION** (개발자 PASS/DONE 선언 금지)
...
| AC-15 사용자 실화면 | 자동 통과만으론 완료 아님 | **미충족 — 사용자 실화면 판정 대기** |
...
- STATE_LATEST 갱신은 **사용자 실화면 PASS(AC-15) 이후**에 수행한다 (설계서 §15 절차: PASS 일 때만 conclusion·상태 문서 갱신). 그 전까지 STATE_LATEST 는 건드리지 않는다 — 현재는 아직 검증 중이므로 정상.
