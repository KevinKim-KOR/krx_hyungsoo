...
- **수신**: 검증자 (Codex)
- **작성일**: 2026-09-04
- **상태**: **`CLOSED`** (2026-09-07). 검증자 r12 `VERIFIED` → **OCI 배포 완료**(`703eb56f`)
  → **운영 실측 확인 완료**(§12).
...
## 12. 운영 실측 — OPS-01A `CLOSED` (2026-09-07)

배포(`703eb56f`) 후 첫 영업일 실행을 확인했다. **코드 변경 0건**, 읽기 전용 대조와
로컬 no-send 재현만 수행했다.

### 12.1 09:15 첫 실행 — 실제 발송

사용자 수신 본문(발췌):

```
[보유 종목 관찰 브리핑] 09:15
기준: 20거래일 전 종가 2026-08-07 · 시세 2026-09-07 09:15

■ 20거래일 하락 8~10% (1)
  KODEX 미국우주항공
      매입대비 -32.1% · 20거래일 -9.1% · 고점 대비 -18.9%

■ 20거래일 하락 5~8% (6)
  KODEX 미국반도체
      매입대비 +91.8% · 20거래일 -6.3% · 고점 대비 -8.5%
  ...
```

| 확인 | 값 |
|---|---|
| status / 길이 | `sent` · **519자** · chunk 1 |
| 선정 | **7종목** (D8_10 1 · D5_8 6) |
| 매입대비 표시 | **정상** — 같은 하락 구간에서 `-32.1%` 와 `+91.8%` 가 갈린다 |
| 두 줄 배치(C안) | 정상 |
| 헤더 | 기준일 `2026-08-07` · 시세 `2026-09-07 09:15` — 두 날짜 구분 표기 정상 |
| state 저장 | **09:15 저장 확인** (`last_slot=OPEN` · 7종목) — 발송 성공 후에만 |

> 사용자 요청으로 추가한 매입대비가 목적을 달성했다. `KODEX 미국우주항공`(매입
> −32.1%)과 `KODEX 미국반도체`(매입 +91.8%)가 **같은 메시지 안에서** 구분된다.

### 12.2 반복 억제 no-send 실측

설계자 확정 조건 그대로 수행 — OCI 09:15 state **복사본** · 현재 quote(29/29,
`price_asof` 전부 2026-09-07) · sender **미전달** · state write **임시 경로**.

| 계약 | 결과 | 근거 |
|---|---|---|
| 변화 있으면 **변화분만** | **OK** | 7종목 중 **신규 1건만** · `full_send=False` · **519자 → 113자** |
| 변화 없으면 **미발송** | **OK** | 갱신된 state 로 재계산 → `skipped/no_change` · 본문 0자 |
| **성공 후에만** state 갱신 | **OK** | `build_holdings_selection` 은 저장하지 않음(임시 state 무변경 확인) |
| 발송 0건 | **OK** | sender 미전달 · 운영 state·DB 무변경 |

변화분 본문:

```
[보유 종목 관찰 브리핑] 12:30 — 변화 1건

■ 새로 선정 (1)
  KIWOOM 미국S&P500&GOLD
      매입대비 -4.0% · 20거래일 -5.2%   (20거래일 하락 5~8%)
```

### 12.3 종류별 차단 실측 (`push_kind_disabled`)

설계자 판정으로 market·spike autosend 를 `false` 로 바꾼 뒤, cron 과 같은 명령을
수동 실행했다.
...
부수 확인 — spike 기록에 **01A 에서 추가한 집합 진단이 실제로 남았다**
(`target_tickers` 20 · `success_tickers` 20 · `failed_tickers` []). market 은
`attempted=0` 으로 가격조회 대상이 아님이 계약대로다.
...
|---|---|
| 검증자 | **`VERIFIED`** (r12) |
| 사용자 형식 확인 | **완료** (목업 `APPROVED` + 실물 수신) |
| OCI 배포 | **완료** (`703eb56f`) |
| 운영 실측 | **완료** (§12.1~12.3) |

→ **OPS-01A `CLOSED`.**

> ⚠️ 12:30·15:40 실제 실행 기록은 **나중에 붙이는 운영 관찰**이며 다음 Step 의
> 차단조건이 아니다(설계자 확정). 억제 계약은 §12.2 로 이미 확인됐다.
