...
- **상태**: `USER_UI_CONFIRMED` · **검증자 2회 REJECTED** · `REMEDIATION_APPLIED — 재검증 대기`
  - UI 는 사용자 실화면 확인 완료("화면 좋습니다. 이대로 진행하면 됩니다.").
  - **전체 판정은 REJECTED 유지.** A11 승인으로 운영 경쟁 조건을 제거했고 재검증을 기다린다.
  - **재검증 PASS 전에는 다음 UI 작업으로 진행하지 않는다**(A11 명시).
