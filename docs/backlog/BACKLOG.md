...
### BACKLOG: ETF 구성 종목 / 정적 데이터 수집
- **보류 사유**: AI 투자세션 복사용 문구 1차를 먼저 사용해보고 구성 종목 정보
  필요성을 판단한다.
- **보류된 위험**: ETF명과 수익률만으로는 AI 의 시장 해석이 얕거나 부정확할 수
  있다. 구성 종목 / 비중 / 카테고리 정적 데이터가 없으면 외부 AI 채널이 "ETF명에
  들어있는 단어 기반 추측" 수준으로 답할 가능성.
- **재검토 트리거**: 사용자가 AI 투자세션을 1회 이상 사용한 뒤 구성 종목 정보가
  없어서 해석 품질이 부족하다고 명시할 때.
- **권장 후속 검토 항목**: 데이터 출처 (FDR / pykrx / KRX OPEN API / 외부 정적 ETF
  메타데이터 등) / 갱신 빈도 / 저장소 (SQLite etf_master 확장 vs 별도 테이블) /
  복사 문구 구조에 구성 종목 섹션 어떻게 통합할지.
...
### BACKLOG: ETF 구성종목 source 후보 — 2026-05-31 Naver 통합 완료
- **2026-05-31 Naver Stock ETFComponent 1차 채택 (DONE)**:
  · source = `naver_stock_etf_component` / endpoint =
    `stock.naver.com/api/domestic/detail/{ticker}/ETFComponent?startIdx=0&pageSize=20`.
  · 국내 종목: componentItemCode 기반. 해외형: componentReutersCode / ISIN /
    market_type 별도 컬럼 보존 + constituent_key 매칭.
  · K6 방어 정책 그대로 (10 cap / cache-first / 0.5s delay / 30s budget /
    partial / unavailable / source 불명 차단).
- **다음 단계 후보 (BACKLOG 유지)**:
  · fetcher 다변화 + chain fallback — Naver 실패 ETF 에 대해 pykrx 또는 KRX
    Open API 자동 fallback. 현재는 단일 fetcher.
  · fetcher timeout 가드 — Naver 는 10s 명시 timeout 있음. pykrx 는 부재.
  · 중복률 임계 기반 경고 — 운영 후 weighted_overlap_pct 가 일정 임계를 넘는
    후보 ETF 쌍이 반복되면 UI 경고 배지.
  · fuzzy 매칭 — 현재는 ticker / reuters / ISIN / 정규화 name 매칭. 한국어
    종목명 표기 변형 (예: "삼성전자" vs "삼성전자보통주") 에 대한 fuzzy.


### BACKLOG: ETF 구성종목 source 후보 (직전 진단 결과 기록) — 2026-05-31
- **2026-05-31 Source Diagnosis 1차 실측**:
  · pykrx PDF — 15 호출 모두 `no_data` → `pykrx_operational_issue` / **hold**.
  · Naver Mobile API — 3 호출 모두 HTTP 404 → **unusable**.
  · 다음 STEP 자동 선정: **C. KRX Open API / Official Provider Source Design**.
  · artifact: `state/market/constituents_source_diagnosis_latest.json` /
    `docs/handoff/ETF_CONSTITUENTS_SOURCE_DIAGNOSIS.md`.
- **현재 운영 영향**: ETF Exposure / Constituents Refresh / Overlap Analysis
  3 기능은 source 미확보로 **사용 불가 (테스트용)** — 인벤토리 §2.10~12 정직
  기록. AI 투자세션 복사용 문구의 [구성종목/중복 노출] 섹션은 자동 회피
  (constituentsAnalysis null 분기).
- **다음 STEP 후보 후속 검토 항목**: KRX Open API 인증키 활성화 / 호출 한도 /
  응답 구조 / 비중 제공 / K6 방어 fit.


### BACKLOG: ETF 구성종목 & 중복률 고도화 — 2026-05-27 신규 (1차 PARTIAL)
- **사용자 요구**: 후보 ETF 가 정말 서로 다른 테마인지, 같은 핵심 종목을 반복
  노출하는 ETF 가 섞여 있는지 시스템이 판단할 수 있어야 한다.
- **상태**: PARTIALLY RESOLVED — ETF Constituents & Overlap 1차에서 pykrx PDF
  fetcher + K6 방어 (10개 cap / cache-first / 0.5s delay / 30s budget /
  partial / source 불명 → unavailable) + 집중도 + 쌍별 중복률 + 반복 핵심
  종목 + AI 문구/AI Sessions 연계 완료.
- **다음 단계 후보**:
  · **fetcher 다변화** — pykrx 실패 시 KRX Open API (인증키 활성화 시) 또는
    운용사 공식 데이터 경로 fallback. 본 1차는 pykrx 단일 source.
  · **fetcher timeout 가드** — 단일 호출 timeout 부재 (기존 FDR timeout
    BACKLOG 와 같은 계열). 외부 source 응답 지연 시 service 의 30s budget
    이 가장 외곽 가드.
  · **중복률 임계 기반 경고** — 운영 후 weighted_overlap_pct 가 일정 임계를
    넘는 후보 ETF 쌍이 반복되면 UI 에 경고 배지.
  · **fuzzy 매칭** — 현재는 ticker 우선 + 정규화된 name 매칭. 한국어 종목명
    표기 변형 (예: "삼성전자" vs "삼성전자보통주") 에 대한 fuzzy 매칭.


### BACKLOG: 시장 국면 판정 고도화 — 2026-05-27 신규 (1차 PARTIAL)
- **사용자 요구 (유지)**: 시스템이 KODEX200/KOSPI 흐름으로 현재 장세를 판정해야
  AI 가 그 위에서 해석/반론을 할 수 있다.
- **상태**: PARTIALLY RESOLVED — Market Regime & Benchmark Context 1차에서
  KODEX200 (필수) / KOSPI (보조) + 20거래일·60거래일 수익률 + MA20/MA60 위치
  점수 합산으로 라벨 (상승장 / 보합장 / 하락장 / 판정불가) 산출. Market
  Discovery / AI 투자세션 복사용 문구 / AI Sessions market_context_snapshot 까지
  연계.
- **다음 단계 후보 (운영 검증 후 진행)**:
  · 변동성 (VKOSPI / KODEX200 표준편차) / 시장 폭 (advance-decline ratio).
  · 장기 추세 (120/200일 이동평균) / 외인·기관 수급.
  · 1차 판정 적중률 검증 — `ai_session_records` 누적 후 라벨이 실제 시장 흐름과
    얼마나 맞았는지 정량 비교.
  · KOSDAQ 비교는 사용자 결정 시점까지 보류 (현재는 코스피 중심 정책).


### BACKLOG: 판단 근거 저장 (decision evidence) — 2026-05-21 갱신 (화면 분리 + 3 채널)
- **사용자 요구 (유지)**: 향후 투자 결과를 판단한 근거를 저장해야 한다. 그래야 과거
  판단과 며칠 뒤 결과를 비교할 수 있다.
- **상태 (2026-05-21 갱신)**: PARTIALLY RESOLVED — 다음까지 구현됨.
  · `state/decision/decision_evidence.sqlite` 분리 저장소 + `ai_session_records`
    테이블 1개 (기존 단일 answer_text → gpt/gemini/claude 3 분리 마이그레이션 완료).
  · `POST /decision/sessions` (3 답변 중 최소 1개 필수) / `GET /decision/sessions`
    (목록에 has_gpt/has_gemini/has_claude) / `GET /decision/sessions/{id}` (3 답변
    분리 응답).
  · Frontend 좌측 메뉴에 AI Sessions 항목 추가. Market Discovery 와 화면 책임
    분리. AI Sessions 화면은 [새 기록 저장] / [기록 조회] 2 탭.
  · Market Discovery → AI Sessions Context Bridge (sessionStorage draft).
  · 질문 / 3 채널 답변 / 메모 / 1차 판정 / 다음 확인 항목 / 후보 스냅샷 / 필터
    스냅샷 / linked_market_refresh_id 저장.
- **복귀 사유**: AI 투자세션 1회가 실제로 발생했고, 그 결과를 저장하지 않으면
  향후 검증과 개선이 불가능.
- **이번 범위 밖 (다음 STEP 분리)**:
  · **매매 결과 추적** — buy / sell / hold 실행 결과와 evidence 연결.
  · **AI 토론 결과 비교 리포트** — 1개월 누적 기록 → 채널별 (GPT vs Gemini
    vs Claude) 어떤 판정이 실제 시장 움직임과 맞았는지 복기.
  · **decision_evidence 의 update / delete** — 본 STEP 은 insert / read 만.
  · **draft 영구 보존** — 본 STEP 의 sessionStorage 는 브라우저 새로고침 시
    소실. 영구 보존이 필요하면 서버 draft 저장소 별도 STEP.
  · **approval 상태 / Telegram message / Run 상태 저장** — 본 STEP 의 범위
    아님. 기존 JSON SSOT 유지.
