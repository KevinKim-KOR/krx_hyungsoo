...
### BACKLOG: 판단 근거 저장 (decision evidence) — 1차 PARTIAL 처리 (2026-05-20)
- **사용자 요구 (유지)**: 향후 투자 결과를 판단한 근거를 저장해야 한다. 그래야 과거
  판단과 며칠 뒤 결과를 비교할 수 있다.
- **상태 (2026-05-20 갱신)**: PARTIALLY RESOLVED — "AI 투자세션 기록 / Decision
  Evidence 1차" STEP 에서 다음까지 구현됨.
  · `state/decision/decision_evidence.sqlite` 분리 저장소 신설 + `ai_session_records`
    테이블 1개.
  · `POST /decision/sessions` / `GET /decision/sessions` / `GET /decision/sessions/{id}`.
  · 질문 / 답변 / 사용자 메모 / 1차 판정 (`useful / needs_constituents /
    needs_market_compare / hold`) / 다음 확인 항목 / 후보 스냅샷 / 필터 스냅샷 /
    linked_market_refresh_id 저장.
  · Market Discovery 안의 `AISessionRecordPanel` 에서 저장 + 최근 10건 목록 +
    상세 조회.
- **복귀 사유**: AI 투자세션 1회가 실제로 발생했고, 그 결과를 저장하지 않으면
  향후 검증과 개선이 불가능 (지시문 §9).
- **이번 범위 밖 (다음 STEP 분리)**:
  · **매매 결과 추적** — buy / sell / hold 실행 결과와 evidence 연결.
  · **AI 토론 결과 비교 리포트** — 1개월 누적 기록 → 어떤 판정이 실제 시장
    움직임과 맞았는지 복기.
  · **decision_evidence 의 update / delete** — 본 STEP 은 insert / read 만.
  · **approval 상태 / Telegram message / Run 상태 저장** — 본 STEP 의 범위
    아님. 기존 JSON SSOT 유지 (PROJECT_ORIGIN_INTENT §10 데이터 종류별
    SSOT 분리).
