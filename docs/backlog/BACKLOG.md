...
- **복귀 조건**: PUSH 1 또는 PUSH 2 구현 Step 1회 이상 완료 후 사용자가 PUSH 3
  진입 결정.
- **참조**: 기존 "급락 임계값" 항목 (OPEN Layer B) 과 통합 처리.
...
- **무릎/머리/어깨 정량 기준**
  - 보류 사유: 보유 종목 추가매수/매도 의견의 핵심 기준이지만 아직 백테스트/운영 데이터 없음.
  - 보류된 위험: 임의 기준을 쓰면 매수/매도 의견이 근거 없는 신호가 됨.
  - 복귀 조건: 보유 종목 의견 PUSH 구현 Step 진입 시.
