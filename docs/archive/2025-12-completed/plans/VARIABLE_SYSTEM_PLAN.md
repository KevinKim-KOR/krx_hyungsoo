...
### Phase 3: UI 변수 관리 ✅ 완료 (2025-12-08)
- [x] UI 변수 설정 패널 (접기/펼치기)
- [x] 변수 활성화/비활성화 토글
- [x] 변수 정보 표시 (범위, step, 카테고리)
- [ ] 변수 범위 슬라이더 (향후 추가)

### Phase 4: 새 변수 실제 구현 (다음 세션)

**목표**: `volatility_filter` 변수를 실제 튜닝/백테스트에 적용

**작업 항목**:
1. [ ] `BacktestParams`에 `volatility_filter` 필드 추가
2. [ ] 변동성 계산 함수 구현 (`core/indicators/volatility.py`)
3. [ ] 백테스트 엔진에 변동성 필터 로직 추가
4. [ ] 튜닝 서비스에서 동적 변수 파라미터 생성
5. [ ] A/B 테스트 (변수 추가 전/후 비교)

**예상 결과**:
- UI에서 `volatility_filter` 활성화 → 튜닝 시 실제 적용
- 변동성 높을 때 진입 제한 → MDD 감소 기대
