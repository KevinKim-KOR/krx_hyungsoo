...
PARAMS = {"ma_short": 60, "ma_long": 120, "adx_period": 30, "adx_threshold": 17.5}
...
def get_file_info(path, name):
    ...
    if not path.exists():
        return {"name": name, "path": str(path), "error": "File Not Found"}
    stat = path.stat()
    return {
        "name": name,
        "path": str(path),
...
def main():
    ...
    if baseline:
        ...
        print(
            f"Coverage Check: Baseline[{b_min}~{b_max}] vs Current[{cur_min}~{cur_max}]"
        )
    ...
    for date in all_dates:
        ...
        for code in integrity_codes:
            integrity_stats["code_counts"][code] = (
                integrity_stats["code_counts"].get(code, 0) + 1
            )

        # Update KPI Stats
        if executed:
            kpi_stats["executed"] += 1
        if data_status != "OK":
            kpi_stats["no_data"] += 1
        if pre_gate == "PASS":
            kpi_stats["gate_open"] += 1
        if pre_reason == "CHOP_BLOCK":
            kpi_stats["chop_block"] += 1
        if pre_reason == "BEAR_BLOCK":
            kpi_stats["bear_block"] += 1

        # --- Construct Recon Row ---
        recon_row = {
            "schema": "RECON_DAILY_V1",
            "trade_date": d_str,
            "l1": {"actions": int(l1_actions)},
            "evidence": {
                "status": data_status,
                "regime": regime,
                "adx": round(adx_val, 2),
            ...
            "precheck": {"gate_decision": pre_gate, "reason": pre_reason},
            "final": {"gate_decision": final_gate, "reason": final_reason},
            "execution": {"executed": bool(executed)},
            "integrity": {"severity": severity, "codes": integrity_codes},
            "pointers": {
                "l1_source": (
                    "phase_c0_daily_ledger_2024_2025.jsonl" if executed else None
                ),
    ...
    # 3.2 Summary JSON
    top_codes = [
        {"code": k, "days": v} for k, v in integrity_stats["code_counts"].items()
    ]
    ...
    summary = {
        ...
        "coverage": {
            ...
            "current": {
                "min_date": (
                    evidence_df.index.min().strftime("%Y-%m-%d")
                    if not evidence_df.empty
                    else "N/A"
                ),
                "max_date": (
                    evidence_df.index.max().strftime("%Y-%m-%d")
                    if not evidence_df.empty
                    else "N/A"
                ),
            },
            "execution_range": {
                ...
                # L1 Map keys > 0
                "min_date": (
                    min([d for d, c in l1_map.items() if c > 0]).strftime("%Y-%m-%d")
                    if any(l1_map.values())
                    else "N/A"
                ),
                "max_date": (
                    max([d for d, c in l1_map.items() if c > 0]).strftime("%Y-%m-%d")
                    if any(l1_map.values())
                    else "N/A"
                ),
            },
