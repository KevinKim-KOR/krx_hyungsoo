...
async def get_ssot_snapshot():
    ...
    if OPS_SUMMARY_PATH.exists():
        try:
            ...
            row = (
                summary["rows"][0] if "rows" in summary and summary["rows"] else summary
            )
    ...
    snapshot = {
        "env_info": {
            ...
            "type": (
                "OCI"
                if "oracle" in platform.node().lower()
                or "instance" in platform.node().lower()
                else "PC"
            ),
        ...
        ),  # P146 Only: Full Ops Summary Sync
...
async def update_ssot_snapshot(
    ...
    allow_tokenless_replay = (
        os.getenv("ALLOW_TOKENLESS_PUSH_IN_REPLAY", "true").lower() == "true"
    )
    ...
    if new_override:
        ...
        ASOF_OVERRIDE_PATH.write_text(
            json.dumps(new_override, indent=2, ensure_ascii=False), encoding="utf-8"
        )
    ...
    if new_bundle:
        ...
        BUNDLE_PATH.write_text(
            json.dumps(new_bundle, indent=2, ensure_ascii=False), encoding="utf-8"
        )
    ...
    if new_guardrails:
        ...
        GUARDRAILS_PATH.write_text(
            json.dumps(new_guardrails, indent=2, ensure_ascii=False), encoding="utf-8"
        )
    ...
    if new_approval:
        ...
        APPROVAL_SNAPSHOT = (
            ...
            / "strategy_bundle"
            / "snapshots"
            / f"live_approval_{datetime.now(KST).strftime('%Y%m%d_%H%M%S')}.json"
        )
        try:
            APPROVAL_LATEST.parent.mkdir(parents=True, exist_ok=True)
            APPROVAL_LATEST.write_text(
                json.dumps(new_approval, indent=2, ensure_ascii=False), encoding="utf-8"
            )
            APPROVAL_SNAPSHOT.parent.mkdir(parents=True, exist_ok=True)
            APPROVAL_SNAPSHOT.write_text(
                json.dumps(new_approval, indent=2, ensure_ascii=False), encoding="utf-8"
            )
    ...
    return {
        ...
        "written_path": (
            "state/strategy_bundle/latest/live_approval.json"
            if approval_written
            else None
        ),
