...
class MarketDataProvider:
    ...
    def fetch_realtime(self, tickers: List[str]) -> Dict[str, Dict]:
        ...
        if to_fetch:
            print(
                f"[MarketData] Fetching {len(to_fetch)} items from {self.provider_type}..."
            )
    ...
    def _fetch_naver_batch(self, tickers: List[str]) -> Dict[str, Dict]:
        ...
        for i in range(0, len(tickers), chunk_size):
            ...
            try:
                url = (
                    f"https://polling.finance.naver.com/api/realtime?query={query_str}"
                )
    ...
    def _fetch_mock(self, tickers: List[str]) -> Dict[str, Dict]:
        ...
        for t in tickers:
            ...
            results[t] = {
                ...
                "nav": (
                    int(price * (1 + random.uniform(-0.01, 0.01)))
                    if "069500" in t
                    else None
                ),
                "name": f"Mock_{t}",
