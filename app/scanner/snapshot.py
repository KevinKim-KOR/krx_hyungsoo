...
def compute_churn_metrics(
    ...
    try:
        ...
        logger.warning(f"[CHURN] 이전 snapshot 로드 실패: {exc}")
        return result
    ...
    if prev_set:
        ratio = len(overlap) / len(prev_set)
    else:
        ratio = 1.0

    result["overlap_ratio"] = round(ratio, 4)
    result["new_entries_count"] = len(new_entries)
    result["removed_entries_count"] = len(prev_set - current_set)

    # Churn 판정
    warnings: List[str] = []
    if ratio < min_overlap_ratio:
        warnings.append(f"overlap_ratio({ratio:.2f}) " f"< min({min_overlap_ratio})")
    if len(new_entries) > max_new_entries:
        warnings.append(f"new_entries({len(new_entries)}) " f"> max({max_new_entries})")

    if warnings:
        result["churn_check_status"] = "warn"
        logger.warning(f"[CHURN] 경고: {'; '.join(warnings)}")
    else:
...
def build_snapshot(
    ...
    config: Dict[str, Any],
    scanner_mode: str = "dynamic_etf_market",
    scanner_version: str = "v1",
    ...
    snapshot_sha = compute_snapshot_sha256(
        ...
        pre_filters={
            ...
            "min_avg_volume_20d": config.get("min_avg_volume_20d", 50000),
    ...
    feature_weights = {f["key"]: f["weight"] for f in active_features if f["enabled"]}
    ...
    return {
        ...
        "removed_entries_count": churn["removed_entries_count"],
        "min_overlap_ratio": min_overlap_ratio,
