...
SEARCH_PATH = (
    BASE_DIR / "reports" / "pc" / "param_search" / "latest" / "param_search_latest.json"
)
PARAMS_PATH = (
    BASE_DIR / "state" / "strategy_params" / "latest" / "strategy_params_latest.json"
)
...
def analyze_candidate(baseline_p, candidate_p, metrics):
    ...
    cand_vol_w = candidate_p.get("weights", {}).get(
        "vol", 0.0
    )  # Search result 'params' structure is flatter: {avg_window, top_k, weights: {mom, vol}} usually
    ...
    return {
        ...
        "regime_hint": (
            ...
            else "AGGRESSIVE" if "AGGRESSIVE" in tags else "NEUTRAL"
        ),
    }, tags
...
def generate_markdown(data):
    ...
    for cand in data["candidates"]:
        ...
        m = cand["metrics"]
        lines.append(
            f"- **Metrics**: Return {m.get('avg_forward_return',0)*100:.1f}%, Hit {m.get('hit_rate',0)*100:.0f}%"
        )
        lines.append("")
...
def main():
    ...
    baseline_info = {
        ...
        "fingerprint": (
            hashlib.sha256(
                json.dumps(current_params, sort_keys=True).encode()
            ).hexdigest()[:16]
            if current_params
            else "UNKNOWN"
        ),
