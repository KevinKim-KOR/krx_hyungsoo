...
def generate_record(input_token: str, items_data: Dict, exec_mode: str = "LIVE"):
    ...
    record = {
        ...
        "decision": "BLOCKED",
        "summary": {"orders_total": 0, "executed_count": 0, "skipped_count": 0},
        "items": [],
        "reason": "UNKNOWN",
        "reason_detail": "",
        "evidence_refs": [],
    ...
    try:
        ...
        record["source"]["ticket_ref"] = str(
            TICKET_LATEST.relative_to(BASE_DIR)
        ).replace("\\", "/")
        record["source"]["plan_id"] = source.get("plan_id")
        record["evidence_refs"] = [
            record["source"]["prep_ref"],
            record["source"]["ticket_ref"],
        ]
    ...
    input_plan_id = items_data.get("source", {}).get(
        "plan_id"
    )  # Input should have source headers
    ...
    if input_plan_id and input_plan_id != prep_plan_id:
        ...
        record["reason_detail"] = (
            f"Input Plan ID {input_plan_id} != Prep Plan ID {prep_plan_id}"
        )
        _save_and_return(record)
        return
    ...
    if (
        current_record
        and current_record.get("linkage", {}).get("plan_id") == prep_plan_id
    ):
        # Same Plan ID exists.
    ...
    record["dedupe"] = {"idempotency_key": input_idem_key}
    ...
    record["linkage"] = {
        "bundle_id": ticket.get("source", {}).get("bundle_id")
        or prep.get("source", {}).get("bundle_id")
        or "UNKNOWN",
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            json.dumps(
                {"decision": "BLOCKED", "reason": "INPUT_ERROR", "error": str(e)}
            )
        )
        sys.exit(1)
