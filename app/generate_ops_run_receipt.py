...
def generate_ops_run_receipt(
    ...
    # Push Delivery Step
    push_delivery_step = {"decision": "SKIPPED", "outbox_ref": None}
    if push_delivery_result:
        push_delivery_step["decision"] = "DONE"
        push_delivery_step["outbox_ref"] = (
            str(OUTBOX_LATEST.relative_to(BASE_DIR)) if OUTBOX_LATEST.exists() else None
        )

    # Live Fire Step
    live_fire_decision, live_fire_reason = determine_live_fire_decision(
        observed_modes, outbox_count
    )
    live_fire_step = {
    ...
    if live_fire_result:
        live_fire_step["decision"] = (
            live_fire_result.get("blocked_reason")
            and "BLOCKED"
            or (live_fire_result.get("attempted") and "DONE" or "SKIPPED")
        )
        live_fire_step["reason"] = live_fire_result.get(
            "blocked_reason", live_fire_reason
        )
        if LIVE_FIRE_LATEST.exists():
            live_fire_step["live_fire_receipt_ref"] = str(
                LIVE_FIRE_LATEST.relative_to(BASE_DIR)
            )
    ...
    if (
        live_fire_result
        and live_fire_result.get("attempted")
        and live_fire_result.get("send_http_status")
    ):
    ...
    receipt = {
        ...
        "run_id": run_id,
        "asof": asof,
        "invocation": {"type": "API", "path": "/api/ops/cycle/run", "caller": caller},
        ...
        "refs": {"ops_run_snapshot": str(snapshot_path.relative_to(BASE_DIR))},
    }
