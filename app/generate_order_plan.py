...
def generate_order_plan(force: bool = False) -> Dict[str, Any]:
    ...
    if "source_bundle" in reco and reco["source_bundle"]:
        ...
        if "integrity" in reco["source_bundle"]:
            report["source"]["payload_sha256"] = reco["source_bundle"]["integrity"].get(
                "payload_sha256"
            )
    ...
    if has_holdings and total_value <= 0:
        ...
        report["reason_detail"] = (
            f"Holdings exist ({len(holdings)}) but Total Value is {total_value}"
        )
        _save_and_return(report)
        return report
    ...
    try:
        ...
        min_cash_pct = (
            bundle.get("strategy", {}).get("position_limits", {}).get("min_cash_pct", 0)
        )
        ...
        if adds and investable_cash > 0:
            ...
            for o in adds:
                ...
                if price > 0:
                    ...
                    raise ValueError(
                        f"Price for {ticker} is 0 or failed to fetch. Halting generation to prevent zero-value allocation."
                    )
        ...
        if breached:
            ...
            report["reason_detail"] = (
                f"Unable to satisfy min_cash_pct floor. Planned: {planned_buy_notional}, Max Budget: {max_buy_budget}"
            )
        ...
        orders = [
            o
            for o in orders
            if not (o["intent"] in ["ADD", "NEW_ENTRY"] and o.get("quantity", 0) <= 0)
        ]
