...
def generate_ops_summary():
    ...
    if ops_run:
        ...
        if (
            status == "DONE" or status == "WARN" or status == "OK"
        ):  # OK/WARN treated as DONE concept
    ...
    if op_decision == "BLOCKED" and not bundle_is_stale:
        ...
        if op_reason == "NO_RECO":
            ...
            if overall_status == "OK":
                overall_status = "WARN"  # or BLOCKED?
    ...
    summary = {
        ...
        "push": {"outbox_row_count": outbox_count, "last_send_decision": send_decision},
