...
def rsi(close: pd.Series, n: int = 14) -> pd.Series:
    ...
    rs = up.ewm(alpha=1 / n, adjust=False).mean() / (
        down.ewm(alpha=1 / n, adjust=False).mean() + 1e-12
    )
    return 100 - (100 / (1 + rs))
...
def adx(high: pd.Series, low: pd.Series, close: pd.Series, n: int = 14) -> pd.Series:
    ...
    plus_di = pd.Series(
        100
        * (
            pd.Series(plus_dm, index=h.index).ewm(alpha=1 / n, adjust=False).mean()
            / atr_
        ),
        index=h.index,
    )
    minus_di = pd.Series(
        100
        * (
            pd.Series(minus_dm, index=h.index).ewm(alpha=1 / n, adjust=False).mean()
            / atr_
        ),
        index=h.index,
    )
...
def turnover_stats(
    close: pd.Series, volume: pd.Series, n: int = 20
) -> Tuple[pd.Series, pd.Series, pd.Series]:
    """
...
def sector_score(
    ...
    for sec in sec_df.columns:
        ...
        # 실제 수익률 계산 (직전 n일 대비)
        short_ret = (
            (s.iloc[-1] / s.iloc[max(len(s) - win_short - 1, 0)]) - 1.0
            if len(s) > win_short
            else np.nan
        )
        long_ret = (
            (s.iloc[-1] / s.iloc[max(len(s) - win_long - 1, 0)]) - 1.0
            if len(s) > win_long
            else np.nan
        )
        score = weights[0] * (short_ret if pd.notna(short_ret) else 0.0) + weights[
            1
        ] * (long_ret if pd.notna(long_ret) else 0.0)
        rows.append(
            {
                "sector": sec,
                "ret_short": short_ret,
