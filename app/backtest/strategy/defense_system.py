...
class DefenseSystem:
    ...
    def __init__(
        ...
        logger.info(
            f"DefenseSystem 초기화: fixed={fixed_stop_loss_pct}%, "
            f"trailing={trailing_stop_pct}%, portfolio={portfolio_stop_loss_pct}%"
        )

    def check_fixed_stop_loss(self, position: Position, current_price: float) -> bool:
        """
        고정 손절 체크
    ...
    def update_trailing_stop(self, position: Position, current_price: float):
        ...
        if current_price > position.peak_price:
            ...
            logger.debug(
                f"트레일링 스톱 업데이트: {position.ticker} "
