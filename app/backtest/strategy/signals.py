...
import pandas as pd
import numpy as np
from app.backtest.infra.indicators import (
    sma,
    ema,
    rsi,
    macd,
...
class SignalGenerator:
    ...
    def generate_momentum_signal(
        ...
        price_vs_ma = (
            (close.iloc[-1] / ma.iloc[-1] - 1.0) * 100 if not ma.empty else 0.0
        )
    ...
    def generate_trend_following_signal(
        ...
        # 3. 가격 모멘텀
        momentum_20 = (
            (close.iloc[-1] / close.iloc[-20] - 1.0) * 100 if len(close) >= 20 else 0.0
        )
        momentum_60 = (
            (close.iloc[-1] / close.iloc[-60] - 1.0) * 100 if len(close) >= 60 else 0.0
        )
    ...
    def generate_maps_signal(
        ...
        if len(close) < self.ma_period:
            return {"signal": "HOLD", "score": 0.0, "confidence": 0.0, "indicators": {}}
