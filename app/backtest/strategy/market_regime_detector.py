...
class MarketRegimeDetector:
    ...
    def __init__(
        ...
        self.bull_threshold = (
            bull_threshold
            if bull_threshold is not None
            else config.get("bull_threshold", 0.02)
        )
        self.bear_threshold = (
            bear_threshold
            if bear_threshold is not None
            else config.get("bear_threshold", -0.02)
        )
        self.trend_strength_period = trend_strength_period or config.get(
    ...
    def calculate_moving_averages(
        self, market_data: pd.DataFrame, current_date: date
        ...
        try:
            ...
            return float(
                short_ma.iloc[0] if hasattr(short_ma, "iloc") else short_ma
    ...
    def detect_regime(
        ...
        try:
            ...
            if regime != prev_regime:
                ...
                logger.info(
                    f"레짐 변경! {current_date}: {prev_regime} → {regime} "
    ...
    def detect_regime_v2(
        self, market_data: pd.DataFrame, current_date: date, ma_period: int = 200
    ) -> Tuple[str, float]:
        """
        ...
        try:
            ...
            if self.current_regime != regime:
                logger.debug(
                    f"Regime V2 Change: {self.current_regime} -> {regime} (Price={current_price:.2f}, MA({ma_period})={ma:.2f})"
                )
    ...
    def _calculate_adx(self, limit_data: pd.DataFrame, period: int = 14) -> float:
        """
    ...
    def get_position_ratio(self, regime: str, confidence: float) -> float:
        """
        레짐에 따른 포지션 비율 계산
        """
        if regime == "bull":
    ...
    def should_enter_defense_mode(self, regime: str, confidence: float) -> bool:
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            **self.stats,
            ...
            "bull_pct": (
                (self.stats["bull_days"] / total_days * 100) if total_days > 0 else 0
            ),
            "bear_pct": (
                (self.stats["bear_days"] / total_days * 100) if total_days > 0 else 0
            ),
            "neutral_pct": (
                (self.stats["neutral_days"] / total_days * 100) if total_days > 0 else 0
            ),
