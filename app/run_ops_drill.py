...
def run_ops_drill() -> Dict[str, Any]:
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            step2 = {
                "name": "ops_cycle",
                "result": (
                    "PASS" if cycle_status in ("DONE", "DONE_WITH_SKIPS") else "WARN"
                ),
    ...
    outbox_file = (
        BASE_DIR / "reports" / "ops" / "push" / "outbox" / "outbox_latest.json"
    )
    ...
    try:
        ...
        if delivery_actual not in (
            ...
            step5["result"] = "FAIL"
            fail_reasons.append(
                f"send_console: delivery_actual={delivery_actual} (expected CONSOLE)"
            )
            overall_result = "FAIL"
    except Exception as e:
        step5 = {
...
def _save_drill_report(
    run_id: str,
    asof: str,
    inputs_observed: Dict,
    steps: List,
    overall_result: str,
    fail_reasons: List,
    evidence_refs: List,
