...
def main():
    ...
    if not is_in_session(
        settings.get("session_kst", {}), settings.get("weekdays", [0, 1, 2, 3, 4])
    ):
    ...
    if not market_map:
        ...
        generate_incident_push(
            "MARKET_DATA_DOWN", "Run Holding Watch", "Empty response from provider"
        )
        sys.exit(3)
    ...
    pnl_down_limit = settings.get(
        "pnl_down_pct", 3.0
    )  # Assume positive input (e.g. 3.0 means -3%)
