...
logging.basicConfig(
    level=logging.INFO, format="[WORKER] %(asctime)s - %(levelname)s - %(message)s"
)
...
def compute_target_proof(path: str, before: dict, after: dict) -> dict:
    ...
    changed = (
        before.get("mtime_iso") != after.get("mtime_iso")
    ...
    if not after.get("exists") or after.get("sha256") is None:
        ...
        verified = (
            ...
            and before.get("sha256") is not None
        )
...
def check_emergency_stop() -> dict:
    try:
        ...
        if resp.status_code == 200:
            ...
            return {
                "passed": not data.get("enabled", False),
...
def load_execution_plan() -> dict:
    if not EXECUTION_PLAN_FILE.exists():
        return {}
    try:
        return json.loads(EXECUTION_PLAN_FILE.read_text(encoding="utf-8")).get(
            "plans", {}
        )
    except:
        return {}
...
def complete_ticket(
    request_id: str, status: str, message: str, artifacts: list = None
) -> bool:
    try:
        resp = requests.post(
...
def write_receipt_v3(
    ...
    logger.info(
        f"Receipt V3 written: {decision}, acceptance.pass={acceptance.get('pass')}"
    )
    return receipt


def process_ticket_real(
    ticket: dict,
    plans: dict,
    window_info: dict,
    safety_checks: dict,
    ...
    targets = []
    for target in PROOF_TARGETS:
        proof = compute_target_proof(
            target, before_snapshots[target], after_snapshots[target]
        )
        targets.append(proof)

    outputs_proof = {"latest_dir": "reports/phase_c/latest/", "targets": targets}
...
def process_ticket(ticket: dict, mode: str) -> bool:
    ...
    if not window_info["passed"]:
        block_reasons.append(
            f"Window not ACTIVE or type not allowed (status: {window_info['status']})"
        )
    ...
    if mode == "MOCK_ONLY":
        ...
        return complete_ticket(
            request_id, "DONE", f"[MOCK_ONLY] Simulated {request_type}"
        )
        ...
        return complete_ticket(
            request_id, "DONE", f"[DRY_RUN] Validated {request_type}"
        )
        ...
        if request_type in ["REQUEST_RECONCILE", "REQUEST_REPORTS"]:
            ...
            if preflight_result.get("decision") != "PREFLIGHT_PASS":
                block_reasons.append(
                    f"Preflight failed: {preflight_result.get('decision')}"
                )
                outputs_proof = {"latest_dir": "reports/phase_c/latest/", "targets": []}
                ...
                return complete_ticket(
                    request_id, "FAILED", f"[PREFLIGHT_FAIL] {block_reasons}"
                )

        # All checks passed - REAL EXECUTE with V3 proof
        return process_ticket_real(
            ticket, plans, window_info, safety_checks, preflight_path
        )
    ...
    try:
        ...
        if success:
            ...
            if receipts_after > receipts_before:
                result["receipt_ref"] = (
                    f"state/tickets/ticket_receipts.jsonl:line{receipts_after}"
                )
        else:
            result["decision"] = "SKIPPED"
