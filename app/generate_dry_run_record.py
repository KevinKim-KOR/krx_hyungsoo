...
def generate_dry_run_record(confirm: bool = False):
    ...
    ticket = json.loads(
        (TICKET_DIR / "manual_execution_ticket_latest.json").read_text(encoding="utf-8")
    )
    ...
    record = {
        ...
        "linkage": {"plan_id": plan_id, "ticket_id": ticket_id},
        "execution_result": "DRY_RUN",
        "decision": "COMPLETED",
        "items": [],  # No items executed in dry run
        "operator_confirm": True,
        "meta": {"generator": "app/generate_dry_run_record.py"},
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--confirm", action="store_true", help="Confirm dry run execution"
    )
