...
def main():
    ...
    if expected_sha and expected_sha != actual_sha:
        print(
            f"[BLOCKED] Integrity mismatch! Expected={expected_sha[:8]}, Actual={actual_sha[:8]}"
        )
        sys.exit(0)
    ...
    print(
        f"PORTFOLIO_OVERRIDE_APPLIED source=BUNDLE sha={actual_sha[:8]} asof={pf_override.get('asof')}"
    )
    ...
    if timing_data:
        ...
        try:
            TIMING_PATH.write_text(
                json.dumps(timing_data, indent=2, ensure_ascii=False), encoding="utf-8"
            )
