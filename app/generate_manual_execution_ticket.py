...
def generate_ticket(force: bool = False):
    ...
    ticket = {
        ...
        "reason_detail": "",
        "orders": [],
        "output_files": {"csv_path": None, "md_path": None},
        "evidence_refs": [],
    }
    ...
    if not prep:
        ticket["decision"] = (
            "BLOCKED"  # Changed from NO_PREP to BLOCKED for consistency
        )
    ...
    if latest_plan_id and latest_plan_id != prep_plan_id:
        ...
        ticket["reason_detail"] = (
            f"Prep based on plan_id {prep_plan_id} but latest Order Plan is {latest_plan_id}"
        )
        _generate_blocked_ticket_content(ticket)
        _save_and_return(ticket)
        return
    ...
    if prep.get("decision") not in ["READY", "WARN"]:
        ...
        ticket["guardrails"] = {
            ...
            "violated": (
                [prep.get("reason")] if prep.get("decision") == "BLOCKED" else []
            ),
        }
    ...
    ticket["guardrails"] = {
        "decision": prep.get(
            "verdict", "UNKNOWN"
        ),  # P120 uses 'verdict' or 'decision'? P120 script uses 'decision' for READY/WARN/BLOCKED.
        "violated": [],
    }
    ...
    if prep.get("decision") == "WARN":
        ticket["guardrails"]["violated"] = [
            "WARNING_TRIGGERED"
        ]  # Placeholder, real codes in safety
...
def _save_and_return(ticket: Dict):
    try:
        ...
        if ticket["decision"] == "GENERATED":
            ...
            shutil.copy(
                str(TICKET_DIR / "latest" / "manual_execution_ticket_latest.csv"),
                str(csv_snap),
            )
            shutil.copy(
                str(TICKET_DIR / "latest" / "manual_execution_ticket_latest.md"),
                str(md_snap),
            )
