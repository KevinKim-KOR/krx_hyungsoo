    ...
    for ref in refs:
        ...
        if result.decision == "OK":
            ...
            stats["fail_reasons"].append(
                {"ref": ref, "decision": result.decision, "reason": result.reason}
            )
...
def run_evidence_health_check() -> Dict[str, Any]:
    ...
    if receipts:
        ...
        resolvable_rate = (
            refs_stats["ok"] / refs_stats["total"] if refs_stats["total"] > 0 else 1.0
        )
    ...
    if send_data:
        ...
        if refs:
            ...
            resolvable_rate = (
                refs_stats["ok"] / refs_stats["total"]
                if refs_stats["total"] > 0
                else 1.0
            )
            decision = calculate_decision(resolvable_rate)
        else:
        ...
        checks.append(
            {
                ...
                "decision": decision,
                "reason": (
                    f"{refs_stats['ok']}/{refs_stats['total']} resolvable"
                    if refs_stats["total"] > 0
        ...
        checks.append(
            {
                ...
                "reason": (
                    f"{refs_stats['ok']}/{refs_stats['total']} resolvable"
                    if refs_stats["total"] > 0
                    else "No evidence_refs (or object format)"
                ),
                "stats": refs_stats,
            }
        )
    ...
    report = {
        ...
        "asof": asof,
        "period": {"from": period_from, "to": asof},
        "summary": {
            "total": len(checks),
            "pass": total_pass,
            "warn": total_warn,
