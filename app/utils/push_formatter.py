...
def check_secret_injection(
    text: str, additional_secret_keys: Optional[List[str]] = None
...
def render_all_channels(
    message: Dict, asof: str, channels: Optional[List[str]] = None
) -> List[Dict]:
    """
    모든 채널에 대해 메시지 렌더링
        ...
        results.append(
            {
                ...
                **render,
                "secret_injection_check": (
                    "PASS" if not render.get("blocked") else "BLOCKED"
                ),
            }
        )
