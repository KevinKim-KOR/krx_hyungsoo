...
def compute_payload_sha256(strategy: Dict) -> str:
    ...
    strategy_json = json.dumps(
        strategy_copy, sort_keys=True, separators=(",", ":"), ensure_ascii=False
    )
...
def validate_strategy_bundle(bundle: Dict) -> BundleValidationResult:
    ...
    if bundle.get("schema") != "STRATEGY_BUNDLE_V1":
        issues.append(
            f"Invalid schema: expected STRATEGY_BUNDLE_V1, got {bundle.get('schema')}"
        )
    ...
    if expected_sha256 != computed_sha256:
        issues.append(
            f"payload_sha256 mismatch: expected={expected_sha256[:16]}..., computed={computed_sha256[:16]}..."
        )
    ...
    rebalance_freq = (
        strategy.get("rebalance_rule", {}).get("frequency", "DAILY").upper()
    )
