...
def print_header():
    ...
    print(
        f"{Colors.GRAY}Checked at: {datetime.now(KST).strftime('%Y-%m-%d %H:%M:%S')}{Colors.RESET}\n"
    )
...
def check_backend():
    ...
    print(
        f"  {Colors.GREEN}● ONLINE{Colors.RESET} (OpsStatus: {ops_status}, Msg: asof={ops_asof})"
    )
...
def fetch_ssot_data(file_path, api_ref):
    ...
    try:
        if file_path:
            ...
            if debug:
                print(f"[DEBUG] Checking SSOT: {abs_path}")
...
def check_watcher_p90(name, file_path, api_ref):
    ...
    print(
        f"  {status_icon} {name:<15} | {status_text:<16} | Reason={reason_enum}, {detail_safe}"
    )
    ...
    if "error" in data and data["error"]:
        ...
        if not rows:
            ...
            if "report" in api_ref:
                ...
                if "human" in api_ref:
                    c5_status = content.get("headline", {}).get(
                        "status_badge", "MISSING_BADGE"
                    )
                else:
                ...
                if status_text == "pass":
                    status_text = "PASS"  # Normalize

                status_icon = f"{Colors.GREEN}●{Colors.RESET}"
                if "FAIL" in status_text.upper():
                    status_icon = f"{Colors.RED}●{Colors.RESET}"
...
def print_order_plan_line(op_data):
    ...
    return (
        ...
        reason_enum,
        detail_safe,
    )  # Return for Root Cause if needed (though Root Cause calculated separately usually)
...
def print_root_cause(summary_data, op_data):
    ...
    if not is_failure:
        ...
        if ops_status not in (
            "OK",
            "WARN",
        ):  # WARN is usually acceptable or handled elsewhere? P89 says Ops != OK
...
def main():
    ...
    check_watcher_p90(
        "Spike Watch",
        "reports/ops/push/spike_watch/latest/spike_watch_latest.json",
        "guard_spike_latest",
    )
    check_watcher_p90(
        "Holding Watch",
        "reports/ops/push/holding_watch/latest/holding_watch_latest.json",
        "guard_holding_latest",
    )
