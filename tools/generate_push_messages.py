...
def generate_market_brief(
    report_human: Optional[Dict], gatekeeper: Optional[Dict]
) -> Dict:
...
def generate_diagnosis_alert(recon_summary: Dict) -> Optional[Dict]:
    ...
    issue_text = "\n".join(
        [f"- {i.get('code', 'UNKNOWN')}: {i.get('message', '')}" for i in issues]
    )
        ...
        actions=[
            ...
            },
            {
                ...
                "ticket_payload": {
                    ...
                    "payload": {
                        "mode": "FULL",
                        "reason": f"Integrity CRITICAL {critical_total}건 감지",
                    },
                },
            },
        ],
