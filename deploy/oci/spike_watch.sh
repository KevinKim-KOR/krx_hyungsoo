...
RESPONSE_RAW=$(curl -s -w "\n%{http_code}" -X POST "http://localhost:8000/api/push/spike/run?confirm=true")
EXIT_CODE=$?
...
# 3. Check for known non-JSON signals (Body Check)
...
if [ -z "$RESULT" ]; then
    echo "[$DATE_STR] ERROR: Parse Failed. Details: $BODY" >> "$LOG_FILE"
