...
# ============================================================================
# POC1 Step 3: Consume approval handoff inbox
# (새 cron 추가 없이 기존 daily_ops 안에서 step 으로 처리. 설계자 결정.)
# ============================================================================
echo ""
echo "$LOG_PREFIX [POC1] Consuming approval inbox..."
if [ -f "deploy/oci/poc1_consume_inbox.sh" ]; then
    bash deploy/oci/poc1_consume_inbox.sh \
        >> logs/poc1_consume.log 2>&1 \
        || echo "$LOG_PREFIX ⚠️ POC1 inbox consume returned non-zero (Non-fatal)"
else
    echo "$LOG_PREFIX ℹ️ poc1_consume_inbox.sh not found (Skipping)"
fi
