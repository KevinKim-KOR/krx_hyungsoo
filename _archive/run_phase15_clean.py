...
def run_clean_phase15(run_id: str, args: argparse.Namespace):
    ...
    def objective(trial):
        params = {
            "ma_period": trial.suggest_int("ma_period", 20, 60, step=10),      # Short MA (Strategy & Cross)
            "rsi_period": trial.suggest_int("rsi_period", 10, 60),
            "stop_loss_pct": trial.suggest_float("stop_loss_pct", 0.05, 0.15, step=0.01),
            "regime_ma_long": trial.suggest_int("regime_ma_long", 120, 200, step=20), # Long Term Regime
            "min_regime_hold_days": trial.suggest_int("min_regime_hold_days", 5, 30, step=5), 
            "adx_period": trial.suggest_int("adx_period", 10, 30, step=5),
            "adx_threshold": trial.suggest_float("adx_threshold", 15.0, 30.0, step=2.5),
        }
