                ...
                if not bt_data:
                    ...
                    if bt_meta.get("dynamic_execution"):
                        ...
                        if bt_meta.get("exo_regime_applied"):
                            ...
                            _fv = load_json(_hv_path) if _hv_path.exists() else {}
                            _fv_agg = _fv.get("aggregate_state", "?")
                            _fv_g = _fv.get("global_state", "?")
                            _fv_d = _fv.get("domestic_state", "?")
                        ...
                        # P209-STEP9A: Drawdown Contribution Summary
                        ...
                        if _dd_peak or _dd_top or _dd_qs:
                            with st.expander(
                                "Drawdown Contribution Summary"
                                " (P209-STEP9A, analysis-only)",
                                expanded=False,
                            ):
                                ...
                                _op_label = _dd_cmp.get(
                                    "operational_baseline_label", "-"
                                )
                                _op_mdd = _dd_cmp.get("operational_mdd_pct")
                                _op_gap = _dd_cmp.get(
                                    "operational_avg_selection_gap_pct"
                                )
                                _b_label = _dd_cmp.get("research_baseline_label", "-")
                                _b_verdict = _dd_cmp.get(
                                    "research_baseline_verdict", "N/A"
                                )
                                ...
                                if _common:
                                    ...
                                    st.caption(
                                        "A/B 공통 Toxic 없음 — 보유구조별"
                                        " 선택 경로가 독립적임"
                                    )

                                # Worst Selection Events side-by-side (gap 포함)
                                _b_worst = (
                                    _dd_cmp.get("research_worst_selection_events") or []
                                )

                                def _worst_rows(evts):
                                    return [
                                        {
                                            "rebalance_date": e.get("rebalance_date"),
                                            "selected": ",".join(
                                                e.get("selected_tickers", []) or []
                                            ),
                                ...
                                _b_bucket = (
                                    _dd_cmp.get("research_bucket_risk_summary") or {}
                                )
                                ...
                                _dd_md_path = (
                                    ...
                                    / "reports"
                                    / "tuning"
                                    / "drawdown_contribution_report.md"
                                )
                                if _dd_md_path.exists():
                                    st.caption(
                                        f"전체 리포트: `{_dd_md_path.name}`"
                                        " — Step9B 필터 설계 근거용"
                                    )
